...
int main(int argc, char **argv)
{
    ...
    if (!ttf_font) ZLOG("could not load arial.ttf\n");
...
void display_proper_init(char *exec_command)
{
    ZLOG("Welcome to the Zod Map Editor\n\n");

    ZLOG("========================================================\n");
    ZLOG("Command list...\n");
    ZLOG("-f filename              - filename to be loaded / saved\n");
    ZLOG("-d dimensions            - dimensions of a new map \n");
    ZLOG("-p palette               - planet palette of a new map\n");
    ZLOG("-m mapname               - mapname of a new map\n");
    ZLOG("-n                       - create map instead of load\n");
    ZLOG("\nEample usage...\n");
    ZLOG("%s -n -f filename.map -d 20x30 -p desert -m virgin_soldiers\n", exec_command);
    ZLOG("%s -f filename.map\n", exec_command);
    ZLOG("========================================================\n");
}
    ...
    {
        {
            ...
            if(!(xi%5))
            {
                snprintf(message, sizeof(message), "%d", xi);
    ...
    {
        {
            ...
            if(!(yi%5))
            {
                snprintf(message, sizeof(message), "%d", yi);
...
void draw_info(bool flip)
{
    ...
    j += 10;
    snprintf(message, sizeof(message), "Current Mode: %s", map_editor_mode_string[current_mode].c_str());
    ...
    {
        case PLACE_TILE_MODE:
            //size() ist size_type, nicht int -- ohne Umwandlung liest %d auf m68k Muell
            snprintf(message, sizeof(message), "Current Object: Palette Tile of %d", (int)current_ptile_list.size());
            break;
        case PLACE_VEHICLE_MODE:
            snprintf(message, sizeof(message), "Current Vehicle: %s", vehicle_type_string[current_object].c_str());
            break;
        case PLACE_CANNON_MODE:
            snprintf(message, sizeof(message), "Current Cannon: %s", cannon_type_string[current_object].c_str());
            break;
        case PLACE_ROBOT_MODE:
            snprintf(message, sizeof(message), "Current Robot: %s", robot_type_string[current_object].c_str());
            break;
        case PLACE_ITEM_MODE:
            snprintf(message, sizeof(message), "Current Item: %s", item_type_string[current_object].c_str());
            break;
        case PLACE_BUILDING_MODE:
            snprintf(message, sizeof(message), "Current Building: %s", building_type_string[current_object].c_str());
            break;
    }
    blit_message(message, x, 390+j, 255, 255, 255);

    j += 10;
    snprintf(message, sizeof(message), "Current Team: %s", team_type_string[current_team].c_str());
    blit_message(message, x, 390+j, 255, 255, 255);
    ...
    switch(current_mode)
    {
        ...
        case PLACE_ITEM_MODE:
            j += 10;
            snprintf(message, sizeof(message), "Current Health Percent: %d", current_health_percent);
            blit_message(message, x, 390+j, 255, 255, 255);
            break;
    }
    ...
    {
        j += 10;
        snprintf(message, sizeof(message), "Current Building Level: %d", current_blevel+1);
        blit_message(message, x, 390+j, 255, 255, 255);

        j += 10;
        snprintf(message, sizeof(message), "Current Bridge Extra Links: %d", current_extra_links);
        blit_message(message, x, 390+j, 255, 255, 255);
    }
    ...
    if(hover_object)
    {
        ...
        if(ot == BUILDING_OBJECT)
            snprintf(message, sizeof(message), "Hover Object: %s L%d", hover_object->GetObjectName().c_str(), hover_object->GetLevel()+1);
        else
            snprintf(message, sizeof(message), "Hover Object: %s", hover_object->GetObjectName().c_str());
        blit_message(message, x, 390+j, 255, 255, 255);
    }
