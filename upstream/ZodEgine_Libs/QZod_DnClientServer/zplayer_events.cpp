...
void ZPlayer::motion_event(ZPlayer *p, char *data, int size, int dummy)
{
	...
	if(p->mouse_x < p->init_w - HUD_WIDTH && p->mouse_y < p->init_h - HUD_HEIGHT)
	{
		...
		//ZLOG("motion:: mouse over map: %d %d \n", map_x, map_y);
		...
		else if(p->lbutton.started_over_gui && p->zcomp_msg.AbsorbedLUnClick(p->mouse_x, p->mouse_y, p->zmap))
		{
			...
			else
			{
				...
				if(obj)
				{
					...
					//ZLOG("zcomp_msg.AbsorbedLUnClick ref_id:%d obj:%d\n", p->zcomp_msg.GetFlags().ref_id, obj);
				}
			}
		}
	}
...
void ZPlayer::keydown_event(ZPlayer *p, char *data, int size, int dummy)
{
	...
	switch(the_key)
	{
		...
		case 27: //esc key
			{
				//ESC beendete frueher SOFORT, und zwar mit einem nackten
				//exit(0) mitten im Tastenhandler. Jetzt kommt dieselbe
				//Rueckfrage wie ueber "Quit Game" im Menue.
				//
				//Der Text steht bewusst NICHT hier zum zweiten Mal, sondern
				//kommt aus GMMMainMenu::QuitWarningFlags() -- sonst liefen
				//die beiden Wege auseinander, sobald einer geaendert wird.
				//Dieselbe Ueberlegung wie bei startbefehl_bauen im Launcher:
				//zwei Stellen, die dasselbe sagen muessen, sind eine zuviel.
				//
				//kill_if_open bleibt false: ein zweites ESC holt die schon
				//offene Rueckfrage nach vorn, statt eine zweite zu oeffnen.
				p->LoadMainMenu(GMM_WARNING, false,
				                GMMMainMenu::QuitWarningFlags());
			}
			break;
		case 305: // rctrl
			p->rctrl_down = true;
...
{
	if(size) ZLOG("ZPlayer::test_event:%s...\n", data);
}
...
void ZPlayer::store_map_event(ZPlayer *p, char *data, int size, int dummy)
{
	...
	if(p->zmap.Loaded())
	{
		...
		/* Zaehler des Statistikbildschirms auf null. HIER und nicht in
		 * ProcessResetGame: eine neue Karte ist der Beginn einer neuen
		 * Runde, und genau dann kommt sie an. Ohne das zaehlten die
		 * Werte ueber die ganze Sitzung weiter -- und das faellt erst
		 * am Ende der ZWEITEN Runde auf. */
		p->stat_getoetet = 0;
		p->stat_verloren = 0;
		p->stat_beginn = p->ztime.ztime;
	}

#ifdef ZOD_ALLOC_PROBE
	zod_alloc_report("client-karte-geladen");
#endif
}
...
void ZPlayer::add_new_object_event(ZPlayer *p, char *data, int size, int dummy)
{
	...
	/* Hier stand ein ZWEITER Aufruf von ReSetupButtons -- die Zeile darunter
	 * ruft ihn ohnehin als erstes (ZPlayer::ProcessChangeObjectAmount).
	 * Das war ein voller Durchlauf ueber object_list je neuem Objekt, umsonst;
	 * auf einer grossen Karte rund 3000 Elemente, und bei einem Robotertrupp
	 * je Einheit erneut. */
...
void ZPlayer::destroy_object_event(ZPlayer *p, char *data, int size, int dummy)
{
	...
	/* Fuer den Statistikbildschirm mitzaehlen. VOR SetHealth, weil der
	 * Besitzer danach noch derselbe ist -- aber vor allem, weil es hier
	 * der einzige Ort ist, an dem der Klient von einem zerstoerten Objekt
	 * erfaehrt UND es noch in der Liste steht. */
	if(obj->GetOwner() == p->our_team) p->stat_verloren++;
	else                               p->stat_getoetet++;
...
void ZPlayer::display_login_event(ZPlayer *p, char *data, int size, int dummy)
{
	...
	{
		//hide it if shown
		if(p->active_menu == p->login_menu) p->active_menu = nullptr;
		if(p->active_menu == p->create_user_menu) p->active_menu = nullptr;
	}
}
...
void ZPlayer::team_ended_event(ZPlayer *p, char *data, int size, int dummy)
{
	...
	/* Der EINZIGE Ort, an dem der Klient erfaehrt, ob WIR gewonnen haben.
	 * END_GAME, das gleich darauf kommt, traegt keine Nutzlast. */
	p->end_video_gewonnen = pi->won;
...
#ifdef ZOD_ALLOC_PROBE
	zod_alloc_report("rundenende-vor-anim");
#endif

	p->zhud.StartEndAnimations(end_animations, pi->won);

#ifdef ZOD_ALLOC_PROBE
	zod_alloc_report("rundenende-nach-anim");
#endif
}
...
void ZPlayer::request_version_event(ZPlayer *p, char *data, int size, int dummy)
{
	...
	{
		ZLOG("ZPlayer::request_version_event: '%s' too large to fit in a %d char array\n", GAME_VERSION, MAX_VERSION_PACKET_CHARS);
		return;
	}
