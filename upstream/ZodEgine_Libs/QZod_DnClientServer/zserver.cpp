#include "zserver.h"
#include "fineclock.h"   /* feine Uhr -- current_time() loest hier 20 ms auf */

/* --- Aufteilung des Postens `server` -------------------------------------
 *
 * `server` ist mit rund 3700 us je Bild (V1200) der zweitgroesste Posten nach
 * dem Zeichnen und war nie aufgeteilt. Die Messpunkte standen bereits hier --
 * `whole_time`, `delete_time`, `process_time`, `path_time`, `update_time` --
 * aber die auswertenden ZLOG-Zeilen waren auskommentiert, und sie hingen an
 * `COMMON::current_time()`. Das ist auf dem Amiga `DateStamp()` mit **20 ms**
 * Aufloesung, also mehr als eine ganze Bildzeit: Jede Einzelmessung war 0 oder
 * 20000 us. Damit war der Posten nicht messbar, nur schaetzbar.
 *
 * DREI LUECKEN, die die alten Klammern liessen und die hier geschlossen sind:
 *   1. `ProcessJobs(0.004)` lag ZWISCHEN `process_time` und `path_time` --
 *      ausgerechnet der Aufruf, der ein Zeitbudget verbrennt.
 *   2. Der Schwanz von ProcessObjects (AddObject, CheckUnitLimitReached,
 *      CheckFlagCaptures) war in keiner Klammer.
 *   3. Die BOT-TICKS liegen hinter `whole_time` und wurden gar nicht gemessen,
 *      obwohl sie im Posten `server` stecken (main.cpp misst die ganze Tick()).
 *
 * Es wird je ABSCHNITTSGRENZE einmal auf die Uhr gesehen, nicht je Abschnitt
 * zweimal -- `ssec_end` schreibt seinen Messpunkt als Beginn des naechsten
 * Abschnitts fort. Auf der V1200 kostet ein Abruf 9882 ns; bei 15 Marken sind
 * das 148 us je Bild, und die Summenzeile weist sie aus. */
enum
{
	SSEC_SOCKET = 0,    /* server_socket.Process                          */
	SSEC_EREIGNIS,      /* ehandler.ProcessEvents                         */
	SSEC_VORSCHLAG,     /* CheckPlayerSuggestions + CheckVoteExpired      */
	SSEC_SCUFFLE,       /* ScuffleUnits (1 Hz, faellt in EIN Bild)        */
	SSEC_LOESCHEN,      /* Loeschschleife (KillMe / DeleteObject)         */
	SSEC_PROCESS,       /* ProcessServer je Objekt                        */
	SSEC_WEGJOBS,       /* ProcessJobs -- der blinde Fleck                */
	SSEC_WEGERGEBNIS,   /* ProcessPathFindingResults                      */
	SSEC_UPDATE,        /* sflags-Auswertungsschleife                     */
	SSEC_NEUE,          /* AddObject, UnitLimit, CheckFlagCaptures        */
	SSEC_MISSILES,      /* ProcessMissiles                                */
	SSEC_ENDE,          /* CheckEndGame/ResetGame/UpdateOnlineHistory     */
	SSEC_BOTS,          /* die Bot-Ticks                                  */
	SSEC_MAX
};

static const char *zod_ssec_name[SSEC_MAX] =
{
	"socket", "ereignis", "vorschlag", "scuffle", "loeschen",
	"process", "wegjobs", "wegergebnis", "update", "neue",
	"missiles", "ende", "bots"
};

static unsigned long zod_ssec[SSEC_MAX];
/* HOECHSTWERT je Abschnitt -- bisher wurde nur summiert, und genau deshalb
 * blieben die Bilder von ueber einer Sekunde unerklaert: Das schlimmste
 * ZEICHENbild meldet 370 ms, die Bildzeit aber 2,4 s. Eine einzelne teure
 * Wegsuche laeuft ganz oder gar nicht -- im Mittel ist sie unsichtbar. */
static unsigned long zod_ssec_max[SSEC_MAX];
static unsigned long zod_ssec_mark = 0;
static unsigned long zod_ssec_takte = 0;
static int           zod_ssec_an = -1;

extern bool zod_time_measuring();   /* zplayer.cpp */

static inline void zod_ssec_begin(void)
{
	if(zod_ssec_an < 0) zod_ssec_an = zod_time_measuring() ? 1 : 0;
	if(!zod_ssec_an) return;

	zod_ssec_mark = zod_fineclock_ticks();
}

static inline void zod_ssec_end(int slot)
{
	if(zod_ssec_an <= 0) return;

	const unsigned long jetzt = zod_fineclock_ticks();

	{
		const unsigned long d = jetzt - zod_ssec_mark;

		zod_ssec[slot] += d;
		if(d > zod_ssec_max[slot]) zod_ssec_max[slot] = d;
	}
...
extern "C" void zod_server_sections_report(long frames)
{
	 ...
	 * Bericht endete mit Guru 80000005 (dieselbe Falle wie beim Zeichnen). */
	if(freq < 1000 || frames <= 0 || !zod_ssec_takte) return;

	unsigned long summe = 0;

	for(int i = 0; i < SSEC_MAX; i++)
	{
		/* Erst teilen, dann malnehmen -- sonst Ueberlauf auf 32 Bit. */
		const unsigned long us = (zod_ssec[i] / frames) * 1000UL / (freq / 1000UL);

		summe += us;

		/* Bei der Spitze erst malnehmen, solange es passt, sonst erst
		 * teilen: `ticks * 1000` laeuft auf 32 Bit ab rund 6 s ueber, und
		 * genau solche Bilder suchen wir hier. */
		const unsigned long sp = (zod_ssec_max[i] < 4000000UL)
		       ? (zod_ssec_max[i] * 1000UL / (freq / 1000UL))
		       : ((zod_ssec_max[i] / (freq / 1000UL)) * 1000UL);

		ZLOG("  server.%s = %ld us, SPITZE %ld us\n",
		     zod_ssec_name[i], (long)us, (long)sp);
	}

	ZLOG("  server.SUMME = %ld us (muss `server` oben entsprechen)\n", (long)summe);
	ZLOG("  server.Takte = %ld bei %ld Bildern\n",
	     (long)zod_ssec_takte, (long)frames);
}
...
void ZServer::Setup()
{
	...
	//Gegenprobe Dezimaltrennzeichen: 0 hier heisst, Kommazahlen wurden
	//nicht gelesen (Fahrzeuge stehen, Waffen ohne Wirkung). Erwartet 3378 / 899 (abgeschnitten).
	ZLOG("Einstellungen: light.health*10000=%d partially_damaged_unit_speed*1000=%d\n",
	     (int)(zsettings.vehicle_settings[LIGHT].health * 10000),
	     (int)(zsettings.partially_damaged_unit_speed * 1000));

	//load the map list
	if(map_list_name.size() && !ReadMapList())
			ZLOG("ZServer::Setup:could not load map list '%s'\n", map_list_name.c_str());
	...
	if(!map_name.size() && !map_list.size())
	{
		...
		ZLOG("ZServer::Setup:using selectable map list as the map list\n");
	}
	...
	if(!server_socket.Start())
		ZLOG("ZServer::Setup:socket not setup\n");
...
void ZServer::InitBots()
{
	for(int i=0;i<MAX_TEAM_TYPES;i++)
	{
		...
		bot[i].SetLoopbackServer(&server_socket);
...
void ZServer::StartBot(int bot_team)
{
	...
	if(bot_active[bot_team])
	{
		ZLOG("ZServer::StartBot: bot already started: %s\n", team_type_string[bot_team].c_str());
		return;
	}

	//kein eigener Thread mehr: der Bot wird aus ZServer::Tick mitgefahren
	bot[bot_team].AllowRun();
	bot[bot_team].Setup();
	bot_active[bot_team] = true;

	ZLOG("ZServer::StartBot: bot started: %s\n", team_type_string[bot_team].c_str());
}
...
void ZServer::KillBot(int bot_team)
{
	...
	if(!bot_active[bot_team]) return;

	bot[bot_team].AllowRun(false);
	bot_active[bot_team] = false;

	ZLOG("ZServer::StartBot: bot stopped: %s\n", team_type_string[bot_team].c_str());
}

void ZServer::KillAllBots()
{
	for(int i=0;i<MAX_TEAM_TYPES;i++) if(bot_active[i]) KillBot(i);
}
...
void ZServer::ProcessEndGame()
{
	...
	server_socket.SendMessageAll(END_GAME, nullptr, 0);
}
...
void ZServer::DoResetGame(string map_name)
{
	...
	//tell them to reset
	server_socket.SendMessageAll(RESET_GAME, nullptr, 0);
}
...
void ZServer::CheckResetGame()
{
    ...
    if(COMMON::current_time() < next_reset_game_time) return;
...
{
#ifdef ZOD_ALLOC_PROBE
	zod_alloc_report("server-reset-anfang");
#endif
	...
	/* Die beiden Sammellisten des laufenden Takts ebenfalls leeren: in
	 * new_object_list stehen Zeiger auf Objekte, die DeleteAllObjects gerade
	 * zerstoert hat, und beide wurden bisher erst im naechsten Takt auf der
	 * NEUEN Karte uebernommen. */
...
void ZServer::LoadNextMap(string override_map_name)
{
	...
	if(map_name.size())
	{
		if(!zmap.Read(map_name.c_str()))
				ZLOG("ZServer::LoadNextMap:could not load map '%s'\n", map_name.c_str());
	}
	else
		ZLOG("ZServer::LoadNextMap:no map set to load\n");
...
int ZServer::NextInMapList()
{
	if(!map_list.size())
	{
		ZLOG("NextInMapList::error, no maps in the map list\n");
		return -1;
	}
...
bool ZServer::ReadMapList()
{
	...
	if(!fp) 
	{
		ZLOG("ReadMapList::could not load '%s'\n", map_list_name.c_str());
		return false;
	}
...
bool ZServer::ReadSelectableMapList()
{
	...
	if(!fp) 
	{
		ZLOG("ReadSelectableMapList::could not load '%s'\n", psettings.selectable_map_list.c_str());
		return false;
	}
...
bool ZServer::ReadSelectableMapListFromFolder(string foldername)
{
	...
	//debug
	//for(int i=0;i<mlist.size(); i++) ZLOG("ReadSelectableMapListFromFolder::map found:%s\n", mlist[i].c_str());
	...
	{
		ZLOG("ReadSelectableMapListFromFolder::no *.map files found in '%s'\n", foldername.c_str());
		return false;
	}
}
...
ZObject *ZServer::CreateRobotGroup(unsigned char oid, int x, int y, int owner, vector<ZObject*> *obj_list, int robot_amount, int health_percent)
{
	...
	//simple check
	if(oid >= MAX_ROBOT_TYPES) return nullptr;
...
{
   	ZObject *new_object_ptr = nullptr;

	//if(ot == MAP_ITEM_OBJECT && oid != FLAG_ITEM) return nullptr;

	//checks
	if(!CreateObjectOk(ot, oid, x, y, owner, blevel, extra_links)) return nullptr;
...
void ZServer::InitObjects()
{
	for(vector<map_object>::iterator i=zmap.GetObjectList().begin(); i!=zmap.GetObjectList().end(); i++)
	{
		...
		if(i->object_type == ROBOT_OBJECT)
			CreateRobotGroup(i->object_id, i->x*16, i->y*16, i->owner, nullptr, -1, i->health_percent);
		else
			CreateObject(i->object_type, i->object_id, i->x*16, i->y*16, i->owner, nullptr, i->blevel, i->extra_links, i->health_percent);
	}
...
bool ZServer::CheckMapObject(map_object &m_obj)
{
	...
	{
		ZLOG("ZServer::CheckMapObject:bad type\n");
		return false;
	}

	if(m_obj.object_id < 0)
	{
		ZLOG("ZServer::CheckMapObject:unused id\n");
		return false;
	}
	...
	case BUILDING_OBJECT:
		...
		{
			ZLOG("ZServer::CheckMapObject:bad id\n");
			return false;
		}
		break;
	case CANNON_OBJECT:
	...
	{
		ZLOG("ZServer::CheckMapObject:bad team\n");
		return false;
	}

	if(m_obj.x < 0 || m_obj.y < 0)
	{
		ZLOG("ZServer::CheckMapObject:bad coordinate\n");
		return false;
	}

	if(m_obj.object_type == BUILDING_OBJECT && (m_obj.blevel < 0 || m_obj.blevel >= MAX_BUILDING_LEVELS))
	{
		ZLOG("ZServer::CheckMapObject:bad blevel\n");
		return false;
	}
...
void ZServer::Run()
{
	//Einzelschritt-Modell: Run() ist nur die Schleife um Tick(). So kann der
	//Singleplayer Server, Bots und Spieler in einem Task abwechselnd fahren.
	while(allow_run)
	{
		Tick();
		uni_pause(10);
...
// 		ZLOG("zserver\n");
		...
		zod_ssec_end(SSEC_SOCKET);

		//process events
		ehandler.ProcessEvents();

		zod_ssec_end(SSEC_EREIGNIS);
		...
		zod_ssec_end(SSEC_VORSCHLAG);

		//scuffle
		ScuffleUnits();

		zod_ssec_end(SSEC_SCUFFLE);

		//run stuff (teilt sich selbst weiter auf)
		ProcessObjects();

		//process missiles
		ProcessMissiles();

		zod_ssec_end(SSEC_MISSILES);
		...
		zod_ssec_end(SSEC_ENDE);

		//Bots laufen im selben Task mit
		//
		//ACHTUNG: Diese Schleife lag frueher HINTER der Zeitnahme und wurde
		//deshalb nie gemessen -- obwohl sie im Posten `server` steckt, den
		//main.cpp ueber die ganze Tick() legt.
		for(int i=0;i<MAX_TEAM_TYPES;i++)
			if(bot_active[i]) bot[i].Tick();

		zod_ssec_end(SSEC_BOTS);
	}
}
...
void ZServer::ProcessObjects()
{
	...
	zod_ssec_end(SSEC_LOESCHEN);

	//process all the objects
	for(obj=object_list.begin(); obj!=object_list.end();obj++) 
		(*obj)->ProcessServer(zmap, ols);

	zod_ssec_end(SSEC_PROCESS);

	//Pfadsuche-Auftraege mit Zeitbudget abarbeiten (statt Thread pro Anfrage)
	//
	//32 ms, nicht die 4 ms des Upstream. Begruendung, auf der V1200 gemessen:
	//
	//Die 4 ms waren auf dem Amiga NIE wirksam -- current_time() loeste 20 ms
	//auf, die Bedingung konnte das Budget gar nicht sehen, faktisch galt
	//"unbegrenzt". Erst seit der E-Clock (20.09.) ist es eine echte Grenze,
	//und seit die Suche fortsetzbar ist, bestimmt sie, wie lange eine Einheit
	//auf ihren Weg wartet: bei 4 ms rund 100 Expansionen je Bild, also fuer
	//eine durchschnittliche Suche (6635 Expansionen) rund 66 Bilder -- fast
	//drei Sekunden. Der Nutzer meldete das als "die Einheiten reagieren nicht
	//mehr, wenn es zu weit weg ist".
	//
	//A/B auf der V1200, gleiche Karte, je 120 s:
	//                       16 ms    32 ms
	//  Bilder ueber 100 ms      9        3
	//  Bilder ueber 50 ms     583      411
	//  fps                   22,1     23,4
	//Das GROESSERE Budget erzeugt WENIGER langsame Bilder -- die Suchen sind
	//frueher fertig, der Wegsucher ist also in weniger Bildern beschaeftigt.
	//Vom Nutzer am Bildschirm bestaetigt: "Mit PFBUDGET 32000 laeuft es
	//fluessig und die Einheiten reagieren schneller auf die Weganweisung."
	//
	//SetEnv ZOD_PFBUDGET <us> stellt den Wert weiterhin um.
	zmap.GetPathFinder().ProcessJobs(0.032);

	zod_ssec_end(SSEC_WEGJOBS);

	//see if any pathfinding responses have come back
	//and process them
	ProcessPathFindingResults();

	zod_ssec_end(SSEC_WEGERGEBNIS);
	...
	//Dieser Schwanz lag frueher in keiner Klammer.
	zod_ssec_end(SSEC_NEUE);
}
...
void ZServer::UpdateObjectDriverHealth(ZObject *obj)
{
	...
	{
		//ZLOG("UpdateObjectDriverHealth::dead\n");
...
void ZServer::RelayVersion(int player)
{
	...
	{
		ZLOG("ZServer::RelayVersion: '%s' too large to fit in a %d char array\n", GAME_VERSION, MAX_VERSION_PACKET_CHARS);
		return;
	}
	...
	if(zvote.StartVote(vote_type, value))
	{
		if(player != -1)
		{
			...
			if(append_description.length())
				snprintf(message, sizeof(message), "vote started by %s to %s: %s", player_info[player].name.c_str(), vote_type_string[vote_type].c_str(), append_description.c_str());
			else
				snprintf(message, sizeof(message), "vote started by %s to %s", player_info[player].name.c_str(), vote_type_string[vote_type].c_str());
...
void ZServer::ProcessVote(int vote_type, int value)
{
	...
	case CHANGE_MAP_VOTE:
		...
		break;
	case START_BOT:
		if(!bot_active[value]) StartBot(value);
...
void ZServer::GivePlayerSelectableMapList(int player)
{
	...
	if(send_str.length())
		...
		server_socket.SendMessage(player, GIVE_SELECTABLE_MAP_LIST, nullptr, 0);
}
...
void ZServer::AttemptPlayerLogin(int player, string loginname, string password)
{
	...
	if(!good_user_string(loginname.c_str()) || !good_user_string(password.c_str()))
	{
		...
		snprintf(message, sizeof(message), "login error: only alphanumeric characters and entries under %d characters long allowed", MAX_PLAYER_NAME_SIZE);
...
void ZServer::AttemptCreateUser(int player, string username, string loginname, string password, string email)
{
	...
	if(psettings.use_mysql)
	{
		...
		if(!user.format_okay())
		{
			...
			snprintf(message, sizeof(message), "create user error: only alphanumeric characters and entries under %d characters long allowed", MAX_PLAYER_NAME_SIZE);
			SendNews(player, message, 0, 0, 0);
...
void ZServer::ChangeGameSpeed(float new_speed)
{
	...
	snprintf(send_str, sizeof(send_str), "game speed changed to %d%%", (int)(ztime.GameSpeed() * 100));
