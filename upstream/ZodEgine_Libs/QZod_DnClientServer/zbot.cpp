...
void ZBot::Setup()
{
	...
	if(loopback_server)
	{
		if(!client_socket.StartLoopback(loopback_server))
			ZLOG("ZBot::Setup:loopback not setup\n");
	}
	else if(!client_socket.Start(remote_address.c_str()))
		ZLOG("ZBot::Setup:socket not setup\n");
}
	...
	while(units_list.size())
	{
		...
		for(vector<ZObject*>::iterator u=units_list.begin(); u!=units_list.end();)
		{
			...
			if(!(*u)->GetAIList().size())
			{
				...
				{
					//ZLOG("ZBot::GiveOutOrders_3: unit does not have any targets...\n");
			...
			//find nearest target
			{
				...
				if(!uc_obj)
				{
					ZLOG("ZBot::GiveOutOrders_3: !uc_obj\n");
					u = units_list.erase(u);
					continue;
				}
			}
			...
			//find that targets nearest unit
			{
				...
				//didn't find one?
				if(!tc_obj) ZLOG("ZBot::GiveOutOrders_3: !tc_obj\n");
				//do we match?
				else if(tc_obj != *u)
				{
					u++;
		...
		if(!order_given)
		{
			//ZLOG("ZBot::GiveOutOrders_3: exited loop with no orders given\n");
			break;
		}
	}
}
