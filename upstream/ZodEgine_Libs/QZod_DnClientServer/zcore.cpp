...
void ZCore::InitEncryption()
{
	...
	/*
	{
		...
		ZLOG("normal:     ");
		for(int i=0;i<16;i++) ZLOG("%02x", (unsigned char)intest[i]);
		ZLOG("\n");

		zencrypt.AES_Encrypt(intest,16,enctest);

		ZLOG("encrypted:  ");
		for(int i=0;i<16;i++) ZLOG("%02x", (unsigned char)enctest[i]);
		ZLOG("\n");

		zencrypt.AES_Decrypt(enctest,16,outtest);

		ZLOG("deencrypted:");
		for(int i=0;i<16;i++) ZLOG("%02x", (unsigned char)outtest[i]);
		ZLOG("\n");
	}
	*/
}
...
bool ZCore::CheckRegistration()
{
	...
	ZLOG("<<<< ---------------------------------------------------------------- >>>>\n");
	ZLOG("<<<<                  REGISTRATION CHECKING DISABLED                  >>>>\n");
	ZLOG("<<<< ---------------------------------------------------------------- >>>>\n");
	is_registered = true;
	...
	{
		ZLOG(">>>> ---------------------------------------------------------------- <<<<\n");
		ZLOG(">>>>                  no registration key file found                  <<<<\n");
		ZLOG(">>>> please visit www.nighsoft.com on how to register your zod engine <<<<\n");
		ZLOG(">>>> ---------------------------------------------------------------- <<<<\n");
	...
	{
		ZLOG(">>>> ---------------------------------------------------------------- <<<<\n");
		ZLOG(">>>>            registration key file unreadable or corrupt           <<<<\n");
		ZLOG(">>>> please visit www.nighsoft.com on how to register your zod engine <<<<\n");
		ZLOG(">>>> ---------------------------------------------------------------- <<<<\n");
	}
...
bool ZCore::CreateObjectOk(unsigned char ot, unsigned char oid, int x, int y, int owner, int blevel, unsigned short extra_links)
{
	...
	{
		ZLOG("CreateObjectOk:: invalid team:%d\n", owner);
		return false;
	}
...
string ZCore::VoteAppendDescription()
{
	...
	case CHANGE_MAP_VOTE:
		...
		snprintf(num_c, sizeof(num_c), "%d. ", value);

		return num_c + COMMON::map_short_name(selectable_map_list[value]);
		break;
	case START_BOT:
	case STOP_BOT:
		if(value < 0) return "";
...
ZObject* ZCore::ProcessWaypointData(char *data, int size, bool is_server, int ok_team)
{
	...
	if(size != expected_packet_size) return nullptr;

	//find our object
	our_object = GetObjectFromID(ref_id, object_list);

	//not found?
	if(!our_object) return nullptr;

	//ok team?
	if(is_server && our_object->GetOwner() == NULL_TEAM) return nullptr;
	if(is_server && our_object->GetOwner() != ok_team) return nullptr;

	//can set waypoints?
	if(is_server && !our_object->CanSetWaypoints()) return nullptr;

	//is this object currently doing a forced move wp?
	//if(is_server && our_object->GetWayPointList().size() && our_object->GetWayPointList().begin()->mode == FORCE_MOVE_WP) return nullptr;

	//this a crane repair or forced move wp (or etc)?
	//if(is_server && !our_object->CanOverwriteWP()) return nullptr;

	//is this object a minion?
	if(is_server && our_object->IsMinion()) return nullptr;
...
ZObject* ZCore::ProcessRallypointData(char *data, int size, bool is_server, int ok_team)
{
	...
	//can set rallypoints?
	if(!our_object->CanSetRallypoints()) return nullptr;
...
extern "C" unsigned long zod_lauf_grenze, zod_lauf_elem, zod_lauf_elem_grenze;
...
bool ZCore::CheckUnitLimitReached()
{
	...
	zod_lauf_elem_grenze += (unsigned long)object_list.size();
