...
class QZOD_DNCLIENTSERVERSHARED_EXPORT ZServer : public ZCore
{
	public:
		ZServer();
		~ZServer();
		...
		void Setup();
		void Run();
		ServerSocket &GetServerSocket() { return server_socket; }
		//Einzelschritt; der Singleplayer ruft Server, Bots und Spieler
		//abwechselnd in einem Task auf
		void Tick();
	private:
		void SetupEHandler();
		...
		int NextInMapList();
	public:
		/* Planet der geladenen Karte. Nur fuer die Auswahl des
		 * Ladebilds -- der Klient kennt ihn zu dem Zeitpunkt noch
		 * nicht (siehe ZPlayer::lade_planet). */
		int GetTerrainType() { return zmap.GetMapBasics().terrain_type; }

		/* Pfad der geladenen Karte. Fuer die Beschriftung des
		 * Ladebilds -- der Name IN der Kartendatei ist unbrauchbar
		 * (dort steht durchweg "clone_map"), der Dateiname dagegen ist
		 * genau das, was auch im Auswahlmenue steht. */
		const string &GetMapFile() { return map_name; }
	...
	private:
		...
		ZObject *CreateObject(unsigned char ot, unsigned char oid, int x, int y, int owner, vector<ZObject*> *obj_list = nullptr, int blevel = 0, unsigned short extra_links = 0, int health_percent = 100);
		ZObject *CreateRobotGroup(unsigned char oid, int x, int y, int owner, vector<ZObject*> *obj_list = nullptr, int robot_amount = -1, int health_percent = 100);
		...
		bool bot_active[MAX_TEAM_TYPES];
