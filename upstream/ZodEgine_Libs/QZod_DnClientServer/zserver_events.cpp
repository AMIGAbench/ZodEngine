...
void ZServer::test_event(ZServer *p, char *data, int size, int player)
{
	...
	if(size) ZLOG("ZServer::test_event:%s... from player %d\n", data, player);
	
	snprintf(message, sizeof(message), "hello player %d", player);
...
void ZServer::connect_event(ZServer *p, char *data, int size, int player)
{
	...
	{
		ZLOG("ZServer::connect_event: tried to add a player that is already on the list [p:%d] vs [size:%s]\n", player, p->player_info.size());
		return;
	}
...
void ZServer::disconnect_event(ZServer *p, char *data, int size, int player)
{
	...
	if(player >= p->player_info.size())
	{
		ZLOG("ZServer::disconnect_event: tried to remove a player that is not on the list [p:%d] vs [size:%s]\n", player, p->player_info.size());
		return;
	}
...
void ZServer::set_player_name_event(ZServer *p, char *data, int size, int player)
{
	...
	snprintf(message, sizeof(message), "player '%s' set their name to '%s'", p->player_info[player].name.c_str(), data);
...
void ZServer::set_player_team_event(ZServer *p, char *data, int size, int player)
{
	...
	//snprintf(message, sizeof(message), "player '%s' set their team to the %s team", p->player_info[player].name.c_str(), team_type_string[the_team].c_str());
	//p->BroadCastNews(message);
...
void ZServer::exit_vehicle_event(ZServer *p, char *data, int size, int player)
{
	...
	if(obj->GetDrivers().size())
	{
		if(obj->GetDriverType() >= 0 && obj->GetDriverType() < MAX_ROBOT_TYPES)
		{
			...
			new_obj = p->CreateRobotGroup(obj->GetDriverType(), x, y, obj->GetOwner(), nullptr, obj->GetDrivers().size());
...
void ZServer::bot_login_bypass_event(ZServer *p, char *data, int size, int player)
{
	...
	{
		ZLOG("ZServer::bot_login_bypass_event:: invalid bot_bypass_size\n");
		return;
	}
