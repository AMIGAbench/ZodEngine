...
class QZOD_DNCLIENTSERVERSHARED_EXPORT ZClient : public ZCore
{
	public:
		ZClient();
		
		void SetRemoteAddress(string ipaddress);
		//gesetzt = Verbindung ohne TCP zum Server im selben Prozess
		void SetLoopbackServer(ServerSocket *server) { loopback_server = server; }
