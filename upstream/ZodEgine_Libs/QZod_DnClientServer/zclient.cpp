...
{
	//ask for the version
	client_socket.SendMessage(REQUEST_VERSION, nullptr, 0);

	//ask for game paused
	client_socket.SendMessage(GET_GAME_PAUSED, nullptr, 0);

	//ask for game speed
	client_socket.SendMessage(GET_GAME_SPEED, nullptr, 0);

	//ask for settings
	client_socket.SendMessage(REQUEST_SETTINGS, nullptr, 0);
	...
	client_socket.SendMessage(REQUEST_SELECTABLE_MAP_LIST, nullptr, 0);
...
{
	ZLOG("ZClient::disconnected from the game server...\n");
}
...
{
   client_socket.SendMessage(REQUEST_OBJECTS, nullptr, 0);
}

void ZClient::RequestZoneList()
{
   client_socket.SendMessage(REQUEST_ZONES, nullptr, 0);
}
...
{
	client_socket.SendMessage(REQUEST_PLAYER_ID, nullptr, 0);
	client_socket.SendMessage(REQUEST_PLAYER_LIST, nullptr, 0);
}
...
void ZClient::SendBotBypassData()
{
	...
	{
		ZLOG("ZClient::SendBotBypassData:: invalid bot_bypass_size\n");
		return;
	}
...
ZObject* ZClient::ProcessNewObject(char *data, int size)
{
	...
	//good packet?
	if(size != sizeof(object_init_packet)) return nullptr;

	//this a ok creation?
	if(!CreateObjectOk(o->object_type, o->object_id, o->x, o->y, o->owner, o->blevel, o->extra_links)) return nullptr;
...
void ZClient::ProcessZoneInfo(char *data, int size)
{
	...
	//ZLOG("ProcessZoneInfo::id:%d owner:%d\n", pi->zone_number, pi->owner);
...
ZObject* ZClient::ProcessObjectTeam(char *data, int size)
{
	...
	//got the header?
	if(size < sizeof(object_team_packet)) return nullptr;

	object_team_packet *pi = (object_team_packet*)data;

	//packet good?
	if(size != sizeof(object_team_packet) + (pi->driver_amount * sizeof(driver_info_s))) return nullptr;

	//will this data crash us?
	if(pi->owner < 0) return nullptr;
	if(pi->owner >= MAX_TEAM_TYPES) return nullptr;
	...
	//ZLOG("ProcessObjectTeam:obj driver set to %s\n", robot_type_string[obj->GetDriver()].c_str());
...
ZObject* ZClient::ProcessObjectAttackObject(char *data, int size)
{
	...
	//good packet?
	if(size != sizeof(attack_object_packet)) return nullptr;
...
ZObject* ZClient::ProcessObjectHealthTeam(char *data, int size)
{
	...
	//good packet?
	if(size != sizeof(object_health_packet)) return nullptr;
...
ZObject* ZClient::ProcessFireMissile(char *data, int size)
{
	...
	//good packet?
	if(size != sizeof(fire_missile_packet)) return nullptr;
...
ZObject* ZClient::ProcessObjectLoc(char *data, int size)
{
	...
	if(size != 4 + sizeof(object_location)) return nullptr;
...
ZObject* ZClient::ProcessBuildingState(char *data, int size)
{
	...
	//good packet?
	if(size != sizeof(set_building_state_packet)) return nullptr;
...
ZObject* ZClient::ProcessObjectLidState(char *data, int size)
{
	...
	//good packet?
	if(size != sizeof(set_lid_state_packet)) return nullptr;
...
ZObject* ZClient::ProcessSetGrenadeState(char *data, int size)
{
	...
	//good packet?
	if(size != sizeof(obj_grenade_amount_packet)) return nullptr;
...
bool ZClient::ProcessSelectableMapList(char *data, int size)
{
	...
	//blah
	//{
	//	ZLOG("loaded selectable map list:\n");
