...
void ZTray::UpdateDisplay()
{
	...
	if(!player_info.size())
	{
		ZLOG("No one is currently connected\n");
		return;
	}
		...
		if(team_players[i].size() || bot_player[i])
		{
			if(bot_player[i])
			{
				if(bot_player_ignored[i])
					ZLOG(string(team_type_string[i] + " team -b: " + team_players[i] + "\n").c_str());
				else
					ZLOG(string(team_type_string[i] + " team +b: " + team_players[i] + "\n").c_str());
			}
			else
				ZLOG(string(team_type_string[i] + " team: " + team_players[i] + "\n").c_str());
		}

	if(spectators.size())
		ZLOG(string("spectators: " + spectators + "\n").c_str());

	if(tray_players)
	{
		snprintf(c_message, sizeof(c_message), "tray players: %d\n", tray_players);
		ZLOG(c_message);
	}

	if(nobodies)
	{
		snprintf(c_message, sizeof(c_message), "nobodies: %d\n", nobodies);
		ZLOG(c_message);
	}
