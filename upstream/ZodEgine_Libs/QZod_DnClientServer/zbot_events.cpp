...
{
	if(size) ZLOG("ZBot::test_event:%s...\n", data);
}
...
void ZBot::destroy_object_event(ZBot *p, char *data, int size, int dummy)
{
	...
	{
		/* Wie beim Spieler (zplayer_events.cpp) -- vorher stand hier nur
		 * RemoveObjectFromList auf object_list plus DeleteObjectCleanUp.
		 * Folge: Das Objekt wurde NIE freigegeben und blieb in den elf
		 * uebrigen Listen von ZOLists stehen, unter anderem in
		 * ols.mobile_olist und ols.flag_olist -- genau den Listen, ueber die
		 * ZBot::ProcessAI jede Sekunde laeuft (zbot.cpp:590, :659). Das
		 * spaeter eintreffende DELETE_OBJECT fand das Objekt in object_list
		 * nicht mehr und raeumte deshalb auch nicht nach.
		 * ACHTUNG, EHRLICH GEHALTEN: Die Folge ist HERGELEITET, nicht
		 * nachgestellt. In einem 90-s-Lauf mit zwei Bots und Gefecht wurde
		 * dieser Zweig KEIN EINZIGES MAL betreten (Sonde, wieder entfernt),
		 * und object_list und prender_olist liefen auch im alten Stand nicht
		 * auseinander. Die Aenderung gleicht den Bot also dem Spieler an --
		 * das ist erkennbar die Absicht --, behebt aber keinen gemessenen
		 * Fehler. Ein Leckpruefer taugt hier ohnehin nicht: die Objekte
		 * haengen weiter in den ols-Listen, sind damit erreichbar und werden
		 * von LeakSanitizer per Bauart nicht gemeldet. */
