...
struct QZOD_DNCLIENTSERVERSHARED_EXPORT news_entry
{
   public:
      char r,g,b;
      string message;
...
struct QZOD_DNCLIENTSERVERSHARED_EXPORT key_event
{
	int the_key;
	int the_unicode;
};

class QZOD_DNCLIENTSERVERSHARED_EXPORT selection_info
{
public:
...
class QZOD_DNCLIENTSERVERSHARED_EXPORT mouse_button_info
{
public:
...
class QZOD_DNCLIENTSERVERSHARED_EXPORT SpaceBarEvent
{
public:
...
class QZOD_DNCLIENTSERVERSHARED_EXPORT ZPlayer : public ZClient
{
	public:
		ZPlayer();
		...
		/* Die Farbtiefe ist NICHT einstellbar -- der Setzer und die Option
		 * -D sind am 24.09. entfallen. Auf dem Amiga sind es 8 Bit, sonst
		 * 32; die Begruendung steht am Setzen von screen_depth im
		 * Konstruktor. */

		/* Bildschirm ohne SDL_DOUBLEBUF oeffnen (-f).
		 * Auf echtem RTG ist das Umschalten ein echter Pufferwechsel -- auf der
		 * Vampire V2 gemessen: 136 us, gegen 64846 us im Emulator, wo SDL
		 * kopiert. Dann zeigt jedes Bild abwechselnd einen anderen Puffer, und
		 * alles, was nicht in JEDEM Bild neu gezeichnet wird, flackert.
		 * Dient dem Gegentest dieser Vermutung. */
		void SetSingleBuffer(bool on) { single_buffer = on; }
		/* -N: kein Skalierer (siehe ZSDL_Surface::SetNoScaler). */
		void SetNoScaler(bool on) { no_scaler = on; }
	...
	private:
		...
		/* Zwischensequenz am Rundenende (Sieg/Niederlage).
		 *
		 * ProcessEndGame ist in ZClient virtuell und leer -- der Klient
		 * tat beim Spielende bisher NICHTS. Hier haengt der Film dran.
		 *
		 * Wer gewonnen hat, steht NICHT im END_GAME-Paket (das ist
		 * leer). Es steht in TEAM_ENDED, das der Server unmittelbar
		 * davor schickt und das die Engine ohnehin schon auswertet.
		 * Deshalb wird es dort gemerkt -- am Drahtformat aendert sich
		 * nichts, und das muss so bleiben: der Port ist
		 * drahtkompatibel zu PC-Zod. */
		void ProcessEndGame();

		/* Die Folge am Rundenende ist DREITEILIG, und die Reihenfolge ist
		 * die des Originals (vom Nutzer gemeldet, 24.09.):
		 *
		 *   SpieleEndFilme()    Sieg- bzw. Niederlagenfilm
		 *   ZeigeStatistik()    der Statistikbildschirm
		 *   SpieleWeiterFilm()  der Aufbruch -- NUR bei "Continue"
		 *
		 * Vorher lief der Aufbruchsfilm unmittelbar hinter dem Siegfilm,
		 * in EINEM Aufruf des Players, und die Statistik kam danach. Das
		 * war falsch: der Statistikbildschirm gehoert zur abgeschlossenen
		 * Runde, der Aufbruch zur naechsten.
		 *
		 * Und deshalb zaehlt auch erst SpieleWeiterFilm die Kampagne
		 * weiter: wer "Retry" waehlt, spielt dasselbe Gebiet noch einmal
		 * und muss beim naechsten Sieg denselben Siegfilm sehen. */
		void SpieleEndFilme();
		void SpieleWeiterFilm();
		void ZeigeStatistik();
		int  StatistikWarten();

		/* Die drei Knoepfe des Statistikbildschirms, in BILDkoordinaten
		 * (0..319). Gesetzt beim Zeichnen, gelesen beim Klick -- so
		 * steht die Trefferflaeche an genau EINER Stelle und kann nicht
		 * von der Beschriftung abweichen. */
		int knopf_x[3], knopf_b[3], knopf_wahl[3], knopf_y_o[3];
		int knopf_unter;

		int KnopfUnter(int bx, int by);
		void ZeichneKnoepfe(int bx, int by);
		bool end_video_gewonnen;

		/* Planet fuer das ORIGINALLADEBILD.
		 *
		 * Der Klient kennt den Planeten erst, wenn die Karte ueber den
		 * Socket da ist -- und das ist NACH Load_Graphics, also nach
		 * dem ganzen Ladevorgang. Im Einzelspieler hat der Server die
		 * Karte aber bereits; run_single_player reicht den Planeten
		 * deshalb vorher herein. Im Netzspiel bleibt es bei -1, und
		 * dann gibt es das bisherige Ladebild.
		 *
		 * Wird NUR fuer die Bildauswahl benutzt -- an der Spiellogik
		 * haengt daran nichts. */
		int lade_planet;
		char lade_name[32];
		int  lade_level;
		bool lade_schirm_steht;

		/* Zaehler fuer den Statistikbildschirm.
		 *
		 * Gezaehlt wird KLIENTENSEITIG aus den DESTROY_OBJECT-Paketen,
		 * die die Engine ohnehin bekommt. Das hat zwei Gruende: am
		 * Drahtformat aendert sich nichts (der Port ist
		 * drahtkompatibel zu PC-Zod), und die Spiellogik wird nicht
		 * angefasst -- es ist reine Buchfuehrung.
		 *
		 * Was "getoetet" heisst: zerstoerte Objekte, die NICHT uns
		 * gehoerten. In einem Spiel mit drei oder mehr Parteien ist
		 * das genauer "feindliche Verluste", nicht "von uns
		 * erledigt" -- wer den Schuss abgegeben hat, steht nicht im
		 * Paket. Fuer zwei Parteien, den Normalfall, ist es dasselbe. */
		int stat_getoetet;
		int stat_verloren;
		double stat_beginn;

		/* Wann Film und Statistik faellig sind. 0 = nichts ansteht.
		 * Wanduhr, nicht ztime -- die Spieluhr steht nach dem
		 * Rundenende. */
		double end_faellig;

		/* Spielvariante -- siehe input_options::story_modus. Vorgabe
		 * true, damit ein Aufruf ohne -G sich verhaelt wie bisher. */
		bool story_modus;

		/* Stand in der Kampagnenfolge des Originals.
		 *
		 *   video_planet  Platz in der KAMPAGNENFOLGE, 0..4
		 *                 (Desert, Volcanic, Arctic, Jungle, City).
		 *                 NICHT der Planet der geladenen Karte -- im
		 *                 Original beginnt jede Kampagne auf der Wueste,
		 *                 unabhaengig davon, wie die Karte aussieht.
		 *   video_stufe   wie viele Gebiete dieses Planeten schon
		 *                 gewonnen sind, 0..3. Eine Niederlage laesst
		 *                 sie stehen: im Original wiederholt man
		 *                 dasselbe Gebiet.
		 */
		int video_planet;
		int video_stufe;
		...
		/* Sammelt die Arbeit von ProcessChangeObjectAmount und fuehrt sie
		 * EINMAL je Bild aus, statt je Paket. Siehe zplayer.cpp. */
		void FlushObjectAmount();
		bool objekt_anzahl_faellig;
	...
	public:
		void SetLadePlanet(int p) { lade_planet = p; }
		void SetStoryModus(bool an) { story_modus = an; }
		void SetFpsAnzeige(bool an) { fps_an = an; }

		/* Kartendatei fuer die Beschriftung des Ladebilds. Pfad und
		 * Endung fallen weg, gross geschrieben wird beim Zeichnen. */
		void SetLadeName(const char *p);
	private:
		void ExitProgram();
		...
		TTF_Font *ttf_font_7;
		int mouse_x, mouse_y;
		/* Letzte Zeigerposition, fuer die das HUD wegen Mausnaehe neu
		 * gezeichnet wurde. Ohne diesen Merker feuerte die Auffrischung in
		 * JEDEM Bild, solange der Zeiger im Randstreifen am HUD lag -- auch
		 * wenn er sich ueberhaupt nicht bewegte. */
		int hud_mouse_x, hud_mouse_y;

		/* Bildratenanzeige oben links -- nur Ziffern, laufend.
		 *
		 * Angezeigt wird die AKTUELLE Bildrate, in jedem Bild neu aus der Dauer
		 * des letzten Bildes gerechnet.
		 *
		 * Die zehn Ziffern werden EINMAL vorgerendert und je Bild nur noch
		 * geblittet. Sonst muesste ZFont::Render bei jeder Aenderung der Zahl
		 * eine neue Flaeche anlegen, fuellen und Zeichen fuer Zeichen blitten --
		 * bei einer Anzeige, die sich staendig aendert, waere das genau die Last,
		 * die sie messen soll.
		 *
		 * Gemessen wird ueber die E-Clock (port/amiga/fineclock.h, 1,41 us).
		 * current_time() loest auf dem Amiga nur 20 ms auf -- ein Bild bei
		 * 60 fps dauert 16 ms und waere damit nicht messbar. */
		ZSDL_Surface fps_digit[10];
		bool fps_digits_ready;

		/* Bildrate anzeigen -- siehe input_options::fps_an. Vorgabe true,
		 * damit ein Aufruf ohne -F sich verhaelt wie bisher. */
		bool fps_an;
		unsigned long fps_last_tick;
		...
		bool is_windowed;
		bool use_opengl;
		bool single_buffer;   /* -f: ohne SDL_DOUBLEBUF oeffnen */
		bool no_scaler;       /* -N: Groesse 1.0, kein Eigendrall, flach */
		//bool music_on;
		int loaded_percent;
