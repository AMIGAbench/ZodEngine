...
bool ZSettings::LoadSettings(string filename)
{
    ...
    if(!fp)
    {
        ZLOG("ZSettings::could not open '%s' to read settings\n", filename.c_str());
        return false;
    }
    ...
    while(fgets(cur_line , buf_size , fp))
    {
        ...
        if(strlen(cur_line))
        {
            ...
            //AmigaOS: fruehere Fassungen haben mit dem Dezimalkomma des
            //System-Locales gespeichert ("0,001105"); atof liest das als 0
            ...
            //ZLOG("ZSettings::LoadSettings:['%s'.'%s'.'%s'='%s']\n", element_type, element, variable, value);
...
bool ZSettings::SaveSettings(string filename)
{
    ...
    if(!fp)
    {
        ZLOG("ZSettings::could not open '%s' to save settings\n", filename.c_str());
        return false;
    }
...
{
    //ZLOG("ZUnit_Settings::ReadEntry:var:'%s' val:'%s'\n", variable.c_str(), value.c_str());
...
ZUnit_Settings &ZSettings::GetUnitSettings(unsigned char ot, unsigned char oid)
{
    ...
    case CANNON_OBJECT:
        ...
        empty_settings= cannon_settings[oid]; //TODO return
        break;
    case VEHICLE_OBJECT:
        if(oid >= MAX_VEHICLE_TYPES) break;
        empty_settings= vehicle_settings[oid];
        break;
    case ROBOT_OBJECT:
        if(oid >= MAX_ROBOT_TYPES) break;
        empty_settings= robot_settings[oid];
        break;
    }
