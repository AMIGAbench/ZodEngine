...
namespace ZPath_Finding_AStar
{
    ...
    /* Liefert true, wenn die Suche FERTIG ist. Bei false ist das Zeitbudget
     * des Bildes aufgebraucht und die Suche wird im naechsten Takt an
     * derselben Stelle fortgesetzt (Zustand steht in den statischen Listen
     * in zpath_finding_old.cpp). */
...
public:
    ...
    //Auftragsschlange statt Thread pro Anfrage: der Server arbeitet sie mit
    //einem Zeitbudget je Tick ab (ProcessJobs), damit nichts blockiert.
    void ProcessJobs(double budget_seconds);
    int PendingJobs() const { return (int)pending_list.size(); }
...
private:
    ...
    vector<ZPath_Finding_Response*> pending_list;
