		...
		for(int t=0;t<crater_types[p] && t<MAX_KNOWN_CRATER_TYPES;t++)
		{
			...
			//load smalls
			{
				for(n=0;1;n++)
				{
					...
					snprintf(filename, sizeof(filename), "assets/planets/craters/crater_small_%s_t%02d_n%02d.png", planet_type_string[p].c_str(), t, n);
					...
					if(n>=MAX_KNOWN_CRATER_N)
					{
						...
						ZLOG("ZMapCraterGraphics::Init:n:%d for %s small type:%d too high (%d)\n", n, planet_type_string[p].c_str(), t, MAX_KNOWN_CRATER_N);
						break;
					}
			...
			//load larges
			{
				for(n=0;1;n++)
				{
					...
					snprintf(filename, sizeof(filename), "assets/planets/craters/crater_large_%s_t%02d_n%02d.png", planet_type_string[p].c_str(), t, n);
					...
					{
						SDL_FreeSurface(surface);
						ZLOG("ZMapCraterGraphics::Init:n:%d for %s large type:%d too high (%d)\n", n, planet_type_string[p].c_str(), t, MAX_KNOWN_CRATER_N);
						break;
					}
