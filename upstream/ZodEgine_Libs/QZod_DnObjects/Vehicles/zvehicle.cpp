...
void ZVehicle::Init()
{
	...
	for(j=0;j<MAX_ANGLE_TYPES;j++)
		for(k=0;k<3;k++)
	{
		snprintf(filename_c, sizeof(filename_c), "assets/units/vehicles/tank_lid_r%03d_n%02d.png", ROTATION[j], k);
	...
	for(i=1;i<MAX_TEAM_TYPES;i++)
		for(j=0;j<MAX_ANGLE_TYPES;j++)
			for(k=0;k<2;k++)
	{
		snprintf(filename_c, sizeof(filename_c), "assets/units/robots/tank_fire_%s_r%03d_n%02d.png", team_type_string[i].c_str(), ROTATION[j], k);
