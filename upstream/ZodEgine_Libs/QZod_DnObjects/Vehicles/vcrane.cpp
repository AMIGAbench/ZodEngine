...
void VCrane::Init()
{
	...
	for(i=1;i<MAX_TEAM_TYPES;i++)
		for(j=0;j<MAX_ANGLE_TYPES;j++)
			for(k=0;k<3;k++)
	{
		snprintf(filename_c, sizeof(filename_c), "assets/units/vehicles/crane/base_%s_r%03d_n%02d.png", team_type_string[i].c_str(), ROTATION[j], k);
	...
	for(j=0;j<MAX_ANGLE_TYPES;j++)
	{
		snprintf(filename_c, sizeof(filename_c), "assets/units/vehicles/crane/crane_r%03d.png", ROTATION_INVERTED[j]);
		crane[j].LoadBaseImage(filename_c);// = IMG_Load_Error(filename_c);
	}
	
	for(j=0;j<8;j++)
	{
		snprintf(filename_c, sizeof(filename_c), "assets/units/vehicles/crane/hook_n%02d.png", j);
	...
	{
		snprintf(filename_c, sizeof(filename_c), "assets/units/vehicles/crane/wasted_%s.png", team_type_string[i].c_str());
