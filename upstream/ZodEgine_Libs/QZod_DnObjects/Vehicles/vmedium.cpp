...
void VMedium::Init()
{
		...
		for(j=0;j<4;j++)
	{
		...
		for(k=0;k<3;k++)
		{
			snprintf(filename_c, sizeof(filename_c), "assets/units/vehicles/medium/base_%s_r%03d_n%02d.png", team_type_string[i].c_str(), ROTATION[j], k);
			...
			snprintf(filename_c, sizeof(filename_c), "assets/units/vehicles/medium/base_damaged_%s_r%03d_n%02d.png", team_type_string[i].c_str(), ROTATION[j], k);
	...
	for(j=0;j<MAX_ANGLE_TYPES;j++)
	{
		snprintf(filename_c, sizeof(filename_c), "assets/units/vehicles/medium/topf_r%03d.png", ROTATION[j]);
		top[j].LoadBaseImage(filename_c);// = IMG_Load_Error(filename_c);
	}
	
	for(k=0;k<8;k++)
	{
		snprintf(filename_c, sizeof(filename_c), "assets/units/vehicles/medium/top_pop_n%02d.png", k);
