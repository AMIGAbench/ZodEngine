...
void VHeavy::Init()
{
		...
		for(j=0;j<4;j++)
	{
		...
		for(k=0;k<3;k++)
		{
			snprintf(filename_c, sizeof(filename_c), "assets/units/vehicles/heavy/base_%s_r%03d_n%02d.png", team_type_string[i].c_str(), ROTATION[j], k);
			...
			snprintf(filename_c, sizeof(filename_c), "assets/units/vehicles/heavy/base_damaged_%s_r%03d_n%02d.png", team_type_string[i].c_str(), ROTATION[j], k);
	...
	for(j=0;j<MAX_ANGLE_TYPES;j++)
	{
		snprintf(filename_c, sizeof(filename_c), "assets/units/vehicles/heavy/top_%s_r%03d.png", team_type_string[i].c_str(), ROTATION[j]);
