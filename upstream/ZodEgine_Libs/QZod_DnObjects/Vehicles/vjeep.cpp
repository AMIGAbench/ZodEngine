...
void VJeep::Init()
{
	...
	{
		snprintf(filename_c, sizeof(filename_c), "assets/units/vehicles/jeep/empty_r%03d.png", ROTATION[j]);
	...
	for(i=1;i<MAX_TEAM_TYPES;i++)
		for(j=0;j<MAX_ANGLE_TYPES;j++)
			for(k=0;k<2;k++)
	{
		snprintf(filename_c, sizeof(filename_c), "assets/units/vehicles/jeep/base_%s_r%03d_n%02d.png", team_type_string[i].c_str(), ROTATION[j], k);
		...
		for(k=0;k<4;k++)
	{
		...
		snprintf(filename_c, sizeof(filename_c), "assets/units/vehicles/jeep/under_r%03d_n%02d.png", ROTATION[j], k);
	...
	for(j=0;j<MAX_ANGLE_TYPES;j++)
	{
		snprintf(filename_c, sizeof(filename_c), "assets/units/vehicles/jeep/fire_r%03d_n00.png", ROTATION[j]);
		turrent[j].LoadBaseImage(filename_c);// = IMG_Load_Error(filename_c);
		
		snprintf(filename_c, sizeof(filename_c), "assets/units/vehicles/jeep/fire_r%03d_n01.png", ROTATION[j]);
