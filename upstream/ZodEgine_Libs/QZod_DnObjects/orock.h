...
class QZOD_DNOBJECTSSHARED_EXPORT ORock : public ZObject
{
	public:
		ORock(ZTime *ztime_, ZSettings *zsettings_ = nullptr, int palette_ = DESERT);
		
		static void Init();
		
		static void SetupRockRenders(ZMap &the_map, const vector<ZObject*> &object_list);
		/* Die Objektliste als REFERENZ, nicht als Wert. Vorher kopierte jeder
		 * Aufruf den ganzen Vektor -- auf einer grossen Karte rund 12 KB
		 * malloc + memcpy + free, und zwar bei JEDEM Stein-Ereignis und beim
		 * Kartenaufbau je Stein. Auf AmigaOS ist gerade das Belegen teuer.
		 * Beide Funktionen LESEN die Liste nur. */
		static void EditRockRender(ZMap &the_map, const vector<ZObject*> &object_list, ZObject *robj, bool do_add);
		...
		bool NeedsPreRender() const { return true; }
