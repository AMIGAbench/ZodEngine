...
ZObject::ZObject(ZTime *ztime_, ZSettings *zsettings_)
{
	...
	/* Ausdruecklich setzen, auch wenn es wie ueberfluessige Hygiene aussieht:
	 * Genau dieses Feld laesst gcc 6.5.0b beim Zusammenlegen der
	 * Mitglieds-Nullsetzungen aus (belegt am Disassemblat, zwei Fundstellen in
	 * diesem Konstruktor, Bytes 974-975). Anders als beim ZMap-Fall ist es nur
	 * ein Flag und kein Zeiger, es zerstoert also keinen fremden Speicher --
	 * aber eine uninitialisierte Laufkennung laesst Einheiten zufaellig mit
	 * Laufgeschwindigkeit starten. */
	is_running = false;
...
void ZObject::InitTypeId(unsigned char ot, unsigned char oid)
{
	...
	if(!zsettings)
	{
		ZLOG("ZObject::InitTypeId:zsettings not set\n");
		return;
	}
...
void ZObject::Init(TTF_Font *ttf_font)
{
	...
	for(i=0;i<10;i++)
	{
		...
		snprintf(filename_c, sizeof(filename_c), "%d", i);
	...
	//for(i=0;i<6;i++)
	//{
	//	snprintf(filename_c, sizeof(filename_c), "assets/sounds/selected_%02d.wav", i);
	//	selected_wav[i] = MIX_Load_Error(filename_c);
	//}

	//for(i=0;i<MAX_ROBOT_TYPES;i++)
	//{
	//	snprintf(filename_c, sizeof(filename_c), "assets/sounds/selected_%s.wav", robot_type_string[i].c_str());
	//	selected_robot_wav[i] = MIX_Load_Error(filename_c);
	//}

	//for(i=0;i<12;i++)
	//{
	//	snprintf(filename_c, sizeof(filename_c), "assets/sounds/acknowledge_%02d.wav", i);
...
void ZObject::RenderAttackRadius(ZMap &zmap, SDL_Surface *dest, vector<ZObject*> &avoid_list)
{
	...
	/* Grenzen des ZEICHENBAREN, in Ausschnittlage. Das ist der Schnitt aus
	 * Ausschnitt und Karte: Ist die Karte schmaler als der Ausschnitt, wird
	 * sie zentriert (ZMap::ClampShift) und links/rechts bleibt schwarzer
	 * Rand. Der Angriffsradius reicht als einziger Zeichenweg regelmaessig
	 * ueber sein Objekt hinaus -- bei einer Einheit nahe dem Kartenrand
	 * spruehte er sonst Punkte in den Rand. */
	int bx0 = 0, by0 = 0, bx1 = view_w, by1 = view_h;
	{
		const int kx0 = -shift_x;
		const int ky0 = -shift_y;
		const int kx1 = kx0 + zmap.GetMapBasics().width  * 16;
		const int ky1 = ky0 + zmap.GetMapBasics().height * 16;
	...
	if(cx + attack_radius < bx0) return;
	if(cy + attack_radius < by0) return;
	if(cx - attack_radius > bx1) return;
	if(cy - attack_radius > by1) return;

	/* Umgebaut, weil dieser Weg bei Gruppenauswahl der teuerste im ganzen
	 * Zeichenpfad war -- er laeuft je Bild fuer JEDES ausgewaehlte Objekt:
	 *
	 *  - 20 Aufrufe von sin/cos je Objekt. Jetzt zwei: der Schritt PI_shift
	 *    ist konstant, die uebrigen Winkel entstehen ueber das
	 *    Additionstheorem (Drehrekursion) aus vier Multiplikationen.
	 *  - 40 einzelne ZSDL_FillRect, jedes mit eigener Farbumrechnung
	 *    (SDL_MapRGB ist im 8-Bit-Pfad eine Suche) und eigenem Zuschnitt.
	 *    Jetzt ein einziges ZSDL_FillDots -- genau der Weg, der bei den
	 *    Wegpunktlinien schon einmal aus demselben Grund eingefuehrt wurde.
	 *
	 * Die Punktmenge und die Pruefung gegen avoid_list bleiben unveraendert,
	 * das Bild ist also dasselbe. */
	SDL_Rect punkte[dots * 4];
	int n_punkte = 0;

	const double schritt_sin = sin(PI_shift);
	const double schritt_cos = cos(PI_shift);
	double s_deg = sin(radius_i);
	double c_deg = cos(radius_i);

	deg = radius_i;

	for(i=0;i<dots && deg <= PI / 2;i++,deg+=PI_shift)
	{
		mx = (int)((attack_radius + 3) * s_deg);
		my = (int)((attack_radius + 3) * c_deg);

		/* Naechster Winkel ohne sin/cos:
		 *   sin(a+d) = sin a * cos d + cos a * sin d
		 *   cos(a+d) = cos a * cos d - sin a * sin d          */
		{
			const double s_neu = s_deg * schritt_cos + c_deg * schritt_sin;
			const double c_neu = c_deg * schritt_cos - s_deg * schritt_sin;

			s_deg = s_neu;
			c_deg = c_neu;
		}

		static const int vz[4][2] = { { 1, 1 }, { -1, -1 }, { -1, 1 }, { 1, -1 } };

		for(int k = 0; k < 4; k++)
		{
			SDL_Rect r;

			r.x = (Sint16)(vz[k][0] * mx + cx);
			r.y = (Sint16)(vz[k][1] * my + cy);
			r.w = 2;
			r.h = 2;

			if(!(r.x >= bx0 && r.x + 2 < bx1)) continue;
			if(!(r.y >= by0 && r.y + 2 < by1)) continue;
			if(WithinAttackRadiusOf(avoid_list, r.x + shift_x, r.y + shift_y)) continue;
	...
	if(n_punkte)
		ZSDL_Surface::ZSDL_FillDots(punkte, n_punkte,
		                            team_color[owner].r,
		                            team_color[owner].g,
		                            team_color[owner].b);
}
...
void ZObject::DoRenderWaypoints(ZMap &the_map, SDL_Surface *dest, vector<ZObject*> &object_list, bool is_rally_points, int shift_x, int shift_y)
{
	...
	{
		/* Denselben Weg nehmen wie die Wegpunktlinien.
		 *
		 * Hier stand eine zweite Kopie derselben Schleife -- mit einem
		 * SDL_FillRect je 2x2-Punkt und ohne jeden Zuschnitt. Sie wurde beim
		 * Umbau uebersehen und blieb damit die teure Fassung. Start und Ende
		 * sind rechnerisch identisch: tx/ty ist bereits (x,y) abzueglich des
		 * Kartenversatzes, das Ziel liegt um (nx-x, ny-y) weiter. */
		RenderWaypointLine((int)tx, (int)ty,
		                   nx - mshift_x, ny - mshift_y, view_h, view_w);
...
/* Schraenkt [jlo,jhi] auf die Durchlaeufe ein, bei denen p0 + j*ps ueberhaupt
 * im Bereich [-1, limit+1] liegen kann. Rueckgabe 0, wenn nichts uebrigbleibt.
 * Der Rand von einem Pixel ist Absicht: der Zuschnitt soll grosszuegig sein,
 * entschieden wird weiterhin punktgenau in der Schleife. */
static int zod_clip_axis(double p0, double ps, int limit, int &jlo, int &jhi)
{
	const double lo = -1.0;
	const double hi = (double)limit + 1.0;

	if(ps > -1e-9 && ps < 1e-9)
		return (p0 >= lo && p0 <= hi) ? 1 : 0;
...
void ZObject::RenderWaypointLine(int sx, int sy, int ex, int ey, int view_h, int view_w)
{
	...
	/* Die Punkte werden gesammelt und blockweise gezeichnet. Ein eigenes
	 * SDL_FillRect je 2x2-Punkt bedeutete bei jedem Bild Hunderte Aufrufe
	 * samt SDL_MapRGB und Sperren des Schirms. */
	const int MAX_DOTS = 128;
	SDL_Rect dots[MAX_DOTS];
	int dot_count = 0;
	...
	//ohne diese Pruefung liefert die Division NaN und (int)NaN ist undefiniert
	if(dist < 1.0) return;

	/* Punkt j liegt bei der Bogenlaenge waypoint_i + 4j. */
	double xs = 4 * dx / dist;
	double ys = 4 * dy / dist;
	double x0 = sx + (dx / dist) * waypoint_i;
	double y0 = sy + (dy / dist) * waypoint_i;
	...
	/* Strecke VOR der Schleife auf den Ausschnitt zuschneiden.
	 *
	 * Die Urfassung zaehlte bis zur vollen Weglaenge und verwarf jeden Punkt
	 * erst, NACHDEM seine Gleitkommaschritte bereits bezahlt waren. Bei einem
	 * weit entfernten Ziel entfiel der Grossteil der Arbeit auf Punkte weit
	 * ausserhalb des Bildes -- das ergab die vom Nutzer gemeldete Kennlinie
	 * "Anzahl Einheiten mal Linienlaenge". Jetzt haengt die Arbeit an der
	 * sichtbaren Laenge, nicht mehr an der Entfernung zum Ziel. */
	int jlo = 0;
	int jhi = maxj - 1;

	if(!zod_clip_axis(x0, xs, view_w, jlo, jhi)) return;
	if(!zod_clip_axis(y0, ys, view_h, jlo, jhi)) return;

	if(jlo < 0) jlo = 0;
	if(jhi > maxj - 1) jhi = maxj - 1;
	if(jlo > jhi) return;

	/* Festkomma 16.16 statt double: je Punkt zwei Additionen und zwei
	 * Schiebungen statt zweier FP-Additionen und zweier FP-Wandlungen.
	 * Nach dem Zuschnitt liegen alle Werte im Bereich des Ausschnitts, ein
	 * Ueberlauf ist damit ausgeschlossen.
	 * Hinweis: ">> 16" rundet ab, das alte "(int)" schnitt zur Null hin ab.
	 * Unterschied gibt es nur fuer Werte in [-1,0), also fuer Punkte, die
	 * knapp links bzw. oberhalb des Bildes liegen -- die werden jetzt
	 * verworfen statt auf Spalte/Zeile 0 gezeichnet. */
	int xf  = (int)((x0 + jlo * xs) * 65536.0);
	int yf  = (int)((y0 + jlo * ys) * 65536.0);
	int xsf = (int)(xs * 65536.0);
	int ysf = (int)(ys * 65536.0);
	...
	for(int j = jlo; j <= jhi; j++)
	{
		...
		if(ix >= 0 && iy >= 0 && ix < view_w && iy < view_h)
		{
			dots[dot_count].x = (Sint16)ix;
			dots[dot_count].y = (Sint16)iy;
			dots[dot_count].w = 2;
			dots[dot_count].h = 2;
			dot_count++;
			...
			{
				ZSDL_Surface::ZSDL_FillDots(dots, dot_count, glvl, glvl, glvl);
				dot_count = 0;
			}
		}
...
int ZObject::ProcessServer(ZMap &tmap, ZOLists &ols)
{
	...
	if(waypoint_list.size())
	{
		...
		case PICKUP_GRENADES_WP:
			...
			break;
		default:
			ZLOG("ZObject::ProcessServer::killing unknown waypoint:%d\n", wp->mode);
			KillWP(wp);
			break;
		}
	}
...
bool ZObject::EstimateMissileTarget(ZObject *target, int &tx, int &ty)
{
	...
	if(d <= 0.00001) 
	{
		ZLOG("EstimateMissileTarget::not solvable\n");
		return false;
	}
	if(isz(a))
	{
		ZLOG("EstimateMissileTarget::a is zero?\n");
		return false;
	}
	...
	//ZLOG("dy2_guts:m:%d dx2_1:%lf\n", missile_speed, (-1 * b - sqrt(d)) / (2 * a));
	//ZLOG("dy2_guts:m:%d dx2_2:%lf\n", missile_speed, (-1 * b + sqrt(d)) / (2 * a));

	if(dy2_guts <= 0.00001) 
	{
		//ZLOG("EstimateMissileTarget::trying other dx2\n");
		...
		if(dy2_guts <= 0.00001) 
		{
			ZLOG("EstimateMissileTarget::bad dy2_guts value\n");
			return false;
		}
	}
	...
	{
		ZLOG("EstimateMissileTarget::bad dd value\n");
		return false;
	}
	...
	if(t < 0)
	{
		//ZLOG("EstimateMissileTarget::2nd\n");
		...
		if(dy2_guts <= 0.00001) 
		{
			ZLOG("EstimateMissileTarget::bad dy2_guts value 2nd\n");
			return false;
		}
		...
		if(t < 0)
		{
			ZLOG("EstimateMissileTarget::bad t value\n");
			return false;
		}
	}
...
void ZObject::ProcessAttackWP(vector<waypoint>::iterator &wp, double time_dif, bool is_new, ZOLists &ols, ZMap &tmap)
{
	...
	else
	{
		...
		{
			/* Ueberwachung: Bleibt die Antwort aus, kehrt diese Funktion FUER
			 * IMMER folgenlos zurueck und die Einheit steht stumm -- ohne jede
			 * Meldung. Genau so sieht es aus, wenn Fahrzeuge sich nicht bewegen
			 * und Einheiten nicht schiessen, waehrend Infanterie normal laeuft:
			 * Fahrzeuge brauchen fuer die Abkuerzung "direkter Weg" einen
			 * breiteren Korridor (bc=2) und landen fast immer in der
			 * A*-Warteschlange, Infanterie fast nie.
			 * Einmal je haengendem Auftrag melden, dann verstummen. */
			if(cur_wp_info.pf_request_time > 0.0 &&
			   ztime->ztime - cur_wp_info.pf_request_time > 5.0)
			{
				ZLOG("Wegsuche: Auftrag %d fuer Objekt %d seit %d s ohne Antwort "
				     "-- Einheit bleibt stehen\n",
				     cur_wp_info.path_finding_id, ref_id,
				     (int)(ztime->ztime - cur_wp_info.pf_request_time));

				cur_wp_info.pf_request_time = 0.0;
			}
		...
		else if(!cur_wp_info.got_pf_response)
		{
			...
			if(cur_wp_info.path_finding_id)
			{
				//Wartebeginn merken -- nur zur Diagnose, siehe pf_request_time
				cur_wp_info.pf_request_time = ztime->ztime;
				StopMove();
			}
			else
			{
...
bool ZObject::ProcessMove(double time_dif, ZMap &tmap, int &stop_x, int &stop_y, bool stoppable)
{
	...
	if(!isz(dx) || !isz(dy))
	{
		...
		if(real_move_speed != previous_real_speed)
		{
			...
			{
				ZLOG("error:previous_real_speed(%lf) not above zero!\n", previous_real_speed);
			}
			dx *= real_move_speed;
			dy *= real_move_speed;
...
void ZObject::RecalcDirection()
{
	...
	ZLOG("ZObject::RecalcDirection():%d\n", new_dir);
...
int ZObject::DirectionFromLoc(float dx, float dy)
{
	...
	//ZLOG("DirectionFromLoc:a:%f dir:%d\n", a, dir);
...
void ZObject::ProcessGroupInfoData(char *data, int size, vector<ZObject*> &object_list)
{
	...
	for(int i=0;i<minions;i++)
	{
		...
		{
			ZLOG("ProcessGroupInfoData:could not find a minion\n");
		}
	}
}
...
void ZObject::PostPathFindingResult(ZPath_Finding_Response* response)
{
	...
	{
		//ZLOG("got a good response - thread_id:%d\n", response->thread_id);
		cur_wp_info.got_pf_response = true;
		cur_wp_info.pf_point_list = response->pf_point_list;
		cur_wp_info.path_finding_id = 0;
		//Antwort da -- Wartezeitmessung beenden, sonst meldet die Ueberwachung
		//in ProcessAttackWP/ProcessMoveWP dauerhaft weiter.
		cur_wp_info.pf_request_time = 0.0;
...
void ZOLists::DeleteAllObjects()
{
    ...
    mobile_olist.clear();
    prender_olist.clear();
    prerender_olist.clear();
    aftereffect_olist.clear();
...
void ZOLists::RemoveObject(ZObject *obj)
{
    ...
    RemoveObjectFromList(obj, &prerender_olist);
    RemoveObjectFromList(obj, &aftereffect_olist);
...
/* Histogramm der Objektarten -- entscheidet, wie viel die Steinschatten und
 * der Bot-Durchlauf wirklich wiegen. Auf der Standardkarte sind 162 von 239
 * Objekten Steine; ob das auf anderen Karten auch gilt, war nie gezaehlt. */
unsigned long zod_objart[MAX_MAP_OBJECT_TYPES];
unsigned long zod_objart_steine = 0;
/* Wer steckt wirklich in prerender_olist? Ein Agent hatte aus der Listengroesse
 * geschlossen, es seien fast nur Steine -- das ist zu pruefen, nicht zu glauben. */
unsigned long zod_objart_vor[MAX_MAP_OBJECT_TYPES];

/* Volle Durchlaeufe ueber object_list im Ereignispfad.
 *
 * Jedes neue und jedes geloeschte Objekt zieht mehrere solcher Durchlaeufe
 * nach sich, und ein Schwall (Robotertrupp, Explosion mit vielen Toten) faellt
 * in EIN Bild. Die Zahl der besuchten Elemente ist hardwareunabhaengig und
 * damit das brauchbare Mass fuer eine Verbesserung -- eine Zeitangabe allein
 * sagt nicht, ob ein Eingriff greift. */
extern "C" {
unsigned long zod_lauf_knoepfe = 0;   /* ZPlayer::ReSetupButtons        */
unsigned long zod_lauf_grenze  = 0;   /* ZCore::CheckUnitLimitReached   */
unsigned long zod_lauf_add     = 0;   /* ZOLists::AddObject, Doppelsuche */
unsigned long zod_lauf_del     = 0;   /* ZPlayer::DeleteObjectCleanUp   */
unsigned long zod_lauf_elem    = 0;   /* besuchte Elemente, alle vier   */
unsigned long zod_lauf_elem_knoepfe = 0;
unsigned long zod_lauf_elem_grenze  = 0;
unsigned long zod_lauf_elem_add     = 0;
unsigned long zod_lauf_elem_del     = 0;
}

extern "C" void zod_lauf_report(void)
{
	const unsigned long n = zod_lauf_knoepfe + zod_lauf_grenze
	                      + zod_lauf_add + zod_lauf_del;

	if(!n) return;

	ZLOG("  Objektlisten-Durchlaeufe: %ld gesamt (Knoepfe %ld, Grenze %ld, "
	     "Add %ld, Loeschen %ld), %ld Elemente besucht\n",
	     (long)n, (long)zod_lauf_knoepfe, (long)zod_lauf_grenze,
	     (long)zod_lauf_add, (long)zod_lauf_del, (long)zod_lauf_elem);
	ZLOG("  davon Elemente: Knoepfe %ld, Grenze %ld, Add %ld, Loeschen %ld\n",
	     (long)zod_lauf_elem_knoepfe, (long)zod_lauf_elem_grenze,
	     (long)zod_lauf_elem_add, (long)zod_lauf_elem_del);
}

extern "C" void zod_objart_report(void)
{
	/* Reihenfolge wie die Aufzaehlung in zmap_structures_old.h:27 */
	static const char *n[MAX_MAP_OBJECT_TYPES] =
	{ "fels", "bruecke", "gebaeude", "kanone",
	  "fahrzeug", "roboter", "tier", "kartenobjekt" };

	for(int i = 0; i < MAX_MAP_OBJECT_TYPES; i++)
		if(zod_objart[i])
			ZLOG("  Objektart %s = %ld\n", n[i], (long)zod_objart[i]);

	/* Steine liegen als MAP_ITEM mit oid ROCK_OBJECT in der Liste, tauchen
	 * oben also unter "kartenobjekt" auf. Genau ihre Zahl ist aber die Frage:
	 * ORock::DoPreRender zeichnet je Stein drei Schattenkacheln. */
	ZLOG("  Objektart davon Steine = %ld\n", (long)zod_objart_steine);

	for(int i = 0; i < MAX_MAP_OBJECT_TYPES; i++)
		if(zod_objart_vor[i])
			ZLOG("  mit Vorzeichnen: %s = %ld\n", n[i], (long)zod_objart_vor[i]);
}
...
void ZOLists::AddObject(ZObject *obj)
{
    ...
    zod_lauf_elem_add += (unsigned long)object_list->size();
    for(vector<ZObject*>::iterator o=object_list->begin(); o!=object_list->end(); o++)
        if(*o == obj)
            return;
    ...
    if(ot < MAX_MAP_OBJECT_TYPES) zod_objart[ot]++;
    /* ACHTUNG, hier stand zuerst `oid == ROCK_OBJECT` -- abgeschrieben von der
     * Zeile daneben, die rock_olist fuellt. Das ist die FALSCHE Aufzaehlung:
     * ROCK_OBJECT ist 0 aus `map_object_type`, ORock setzt aber ROCK_ITEM aus
     * der Item-Aufzaehlung (constants.h:91). `oid == ROCK_OBJECT` trifft damit
     * FLAG_ITEM. Der Fehler steckt seit jeher auch in rock_olist -- folgenlos,
     * weil die Liste im ganzen Baum keinen Leser hat. */
    if(ot == MAP_ITEM_OBJECT && oid == ROCK_ITEM) zod_objart_steine++;
    ...
    /* Hier stand die CANNON_OBJECT-Zeile ein ZWEITES Mal. Folgenlos, weil
     * cannon_olist im ganzen Baum keinen Leser hat und
     * RemoveObjectFromList ohnehin ALLE Treffer entfernt -- entfernt als
     * Hygiene, nicht als Fehlerbehebung. */
    ...
    {
        prerender_olist.push_back(obj);
        if(ot < MAX_MAP_OBJECT_TYPES) zod_objart_vor[ot]++;
    }
    if(obj->NeedsAfterEffects()) aftereffect_olist.push_back(obj);
...
void ZPortrait_Unit_Graphics::Load(robot_type oid, team_type team, ZPortrait_Unit_Graphics &bc)
{
    ...
    default:
        ZLOG("ZPortrait_Unit_Graphics::invalid robot id!\n");
        return;
        break;
    }
    ...
    //load em
    i=0;
    snprintf(filename_c, sizeof(filename_c), "assets/other/hud/portraits/%s_%s/SHEADBI%d_%04d.png",  robot_type_string[oid].c_str(), team_type_string[team].c_str(), old_z_face_id, i++);
