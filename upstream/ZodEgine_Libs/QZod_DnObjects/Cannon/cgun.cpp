...
void CGun::Init()
{
	...
	for(i=1;i<MAX_TEAM_TYPES;i++)
		for(j=0;j<4;j++)
	{
		snprintf(filename_c, sizeof(filename_c), "assets/units/cannons/gun/place_%s_n%02d.png", team_type_string[i].c_str(), j);
	...
	for(i=1;i<MAX_TEAM_TYPES;i++)
		for(j=0;j<MAX_ANGLE_TYPES;j++)
	{
		snprintf(filename_c, sizeof(filename_c), "assets/units/cannons/gun/equiped_%s_r%03d.png", team_type_string[i].c_str(), ROTATION[j]);
