...
public:
    ...
    /* Zeitpunkt, an dem der Wegwunsch abgesetzt wurde -- nur zur Diagnose.
     *
     * Bleibt die Antwort aus, kehrt ProcessAttackWP/ProcessMoveWP bei
     * "if(cur_wp_info.path_finding_id) return;" FUER IMMER folgenlos zurueck:
     * die Einheit steht stumm, ohne jede Meldung. Genau so sieht es aus, wenn
     * Fahrzeuge sich nicht bewegen und Einheiten nicht schiessen, waehrend
     * Infanterie normal laeuft -- Fahrzeuge brauchen fuer die Abkuerzung
     * "direkter Weg" einen breiteren Korridor und landen fast immer in der
     * A*-Warteschlange.
     *
     * waypoint_information wird NIE gesendet (kein memcpy, kein sizeof, keine
     * Sendestelle -- nachgeprueft), ein zusaetzliches Feld ist hier also
     * unbedenklich. */
    double pf_request_time;
...
extern const string portrait_anim_string[MAX_PORTRAIT_ANIMS];
