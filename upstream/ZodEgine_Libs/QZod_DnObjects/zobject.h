...
// 		virtual SDL_Surface *GetRender();
		...
		/* Hat diese Klasse ueberhaupt ein eigenes DoPreRender?
		 *
		 * Nur DREI von rund 38 Objektklassen haben eines (ORock, BFort,
		 * OHut). Die Schleife in ZPlayer::RenderObjects lief aber ueber ALLE
		 * Objekte -- im Mittel 393, auf der groessten Karte 3004 -- und rief
		 * fuer die uebrigen eine leere Funktion. Auf der V1200 kostete das
		 * 792 us je Bild, also rund 2 us je Objekt: Der Aufruf selbst ist
		 * billig, aber jedes Objekt liegt woanders im Speicher, und schon
		 * das Holen des vtable-Zeigers ist ein Fehlzugriff im Zwischenspeicher.
		 *
		 * Dieselbe Sache wie bei RenderGuns, das ueber alle Objekte lief und
		 * alles verwarf, was kein Gebaeude war.
		 *
		 * ACHTUNG: WER `DoPreRender` UEBERSCHREIBT, MUSS AUCH DAS HIER
		 * UEBERSCHREIBEN. Sonst landet die Klasse nicht in
		 * `ZOLists::prerender_olist` und ihr Vorzeichnen faellt still aus --
		 * kein Fehler, keine Meldung, nur ein fehlendes Bild. */
		virtual bool NeedsPreRender() const { return false; }

		/* Dasselbe fuer DoAfterEffects. Hier greift der Filter STAERKER: Ohne
		 * eigene Fassung sind Steine, Kartenobjekte, Huetten, Flaggen,
		 * Granaten, Raketen, ALLE vier Kanonentypen und Bruecken -- also die
		 * grosse Mehrheit der Objekte. Eine eigene haben nur Forts, die
		 * Produktionsgebaeude, Radar und Reparatur sowie alle Roboter und
		 * Fahrzeuge.
		 *
		 * Auch hier gilt: WER `DoAfterEffects` UEBERSCHREIBT, MUSS DAS HIER
		 * EBENFALLS UEBERSCHREIBEN. */
		virtual bool NeedsAfterEffects() const { return false; }
		...
		ZObject* GetGroupLeader()
		{
			return leader_obj;
		}
		vector<ZObject*> &GetMinionList()
		{
			return minion_list;
		}
		...
		vector<driver_info_s> &GetDrivers()
		{
			return driver_info;
		}
		...
		void SetDestroyed(bool is_destroyed)
		{
		...
		map_zone_info *GetConnectedZone()
		{
			return connected_zone;
		}
		void SetConnectedZone(map_zone_info *connected_zone_)
		{
...
public:
    ...
    /* Nur die Objekte mit eigenem DoPreRender (ORock, BFort, OHut). Siehe
     * ZObject::NeedsPreRender. */
    vector<ZObject*> prerender_olist;
    /* Nur die Objekte mit eigenem DoAfterEffects (Gebaeude, Roboter,
     * Fahrzeuge). Siehe ZObject::NeedsAfterEffects. */
    vector<ZObject*> aftereffect_olist;
