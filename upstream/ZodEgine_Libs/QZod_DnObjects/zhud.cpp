...
void HubButton::Render(SDL_Surface *dest, int off_x, int off_y)
{
	...
	button_img[state].BlitSurface(nullptr, &to_rect);
	//button_img[state].RenderSurface(x + off_x, y + off_y);
	//SDL_BlitSurface(button_img[state], nullptr, dest, &to_rect);
}
...
ZHud::ZHud()
{
	//unit_amount_text = nullptr;
	//chat_message_img = nullptr;

	render_icon = nullptr;
	render_label = nullptr;
	render_unit_label = nullptr;
	//render_backdrop = nullptr;
...
void ZHud::DoRender(SDL_Surface *dest, int screen_w, int screen_h)
{
	...
	if(rerender_main)
	{
		//render bottom
		{
			...
			//		bottom_filler.BlitSurface(nullptr, &to_rect);
			//}

			//to_rect.x = off_x;
			//to_rect.y = off_y + 448;
			//
			//main_hud_bottom.BlitSurface(nullptr, &to_rect);

			//left
			to_rect.x = 0;
			to_rect.y = off_y + 448;
			main_hud_bottom_left.BlitSurface(nullptr, &to_rect);
			...
			if(main_hud_bottom_right.GetBaseSurface())
			{
				...
				to_rect.y = off_y + 448;
				main_hud_bottom_right.BlitSurface(nullptr, &to_rect);
			}
		...
		//render right side
		{
			if(side_filler.GetBaseSurface())
			{
				...
				for(;to_rect.y < off_y; to_rect.y += side_filler.GetBaseSurface()->h)
					side_filler.BlitSurface(nullptr, &to_rect);
			}
			
			to_rect.x = off_x + 548;
			to_rect.y = off_y;
			
			main_hud_side[team].BlitSurface(nullptr, &to_rect);
		}
	...
	if(rerender_icon)
	{
		...
		if(render_icon)
		{
			...
			render_icon->BlitSurface(nullptr, &to_rect);
			//render_icon->RenderSurface(to_rect.x, to_rect.y);
			//SDL_BlitSurface(render_icon, nullptr, dest, &to_rect);
		}
		...
		if(render_unit_label)
		{
			...
			render_unit_label->BlitSurface(nullptr, &to_rect);
			//render_unit_label->RenderSurface(to_rect.x, to_rect.y);
			//SDL_BlitSurface(render_unit_label, nullptr, dest, &to_rect);
		}
		else
		{
			//clear
		...
		if(render_label)
		{
			...
			render_label->BlitSurface(nullptr, &to_rect);
			//render_label->RenderSurface(to_rect.x, to_rect.y);
			//SDL_BlitSurface(render_label, nullptr, dest, &to_rect);
		}
		else
		{
			//clear
		...
		if(selected_object && selected_object->CanHaveGrenades())
		{
			...
			if(last_grenade_amount != selected_object->GetGrenadeAmount())
			{
				...
				snprintf(grenade_c, sizeof(grenade_c), "%02d", last_grenade_amount);
			...
			grenade_img[selected_object->GetOwner()].BlitSurface(nullptr, &to_rect);

			to_rect.x += 25;
			to_rect.y += 2;

			grenade_amount_surface.BlitSurface(nullptr, &to_rect);
		}
	}
	...
	/* Auch ausserhalb von rerender_main aufrufen. Vorher stand der einzige
	 * Aufruf INNERHALB von if(rerender_main) (zhud.cpp weiter oben) --
	 * rerender_unit_amount wurde bei jeder Aenderung gesetzt, aber niemand
	 * wertete die Marke aus, solange kein vollstaendiger HUD-Neuaufbau
	 * anstand. Die Einheitenzahl im HUD frischte deshalb verspaetet auf.
	 * Die Funktion steigt bei nicht gesetzter Marke sofort aus. */
	RenderUnitAmountBar(dest, off_x, off_y);
}
...
void ZHud::RenderChatMessage(SDL_Surface *dest, int off_x, int off_y)
{
	...
	/* Die Marke wurde NIE zurueckgesetzt -- gesetzt an zwei Stellen
	 * (zhud.cpp:167 und :1204), geprueft hier, und danach lief der
	 * Chatstreifen (rund 340x18 Punkte: Fuellen plus Textblit) in JEDEM Bild
	 * neu, obwohl der ganze uebrige HUD ueber Schmutzmarken laeuft.
	 *
	 * Hier und nicht frueher, damit ein Streifen ohne Platz (clear_w <= 0)
	 * die Marke nicht verbraucht und spaeter doch noch gezeichnet wird.
	 * Sicher, weil der HUD in die Zeichenflaeche zeichnet, die zwischen
	 * Bildern erhalten bleibt -- der Kartenblit deckt nur den linken
	 * Bereich ab. */
	rerender_chat = false;
	...
	if(show_chat && chat_message_img.GetBaseSurface() && (clear_w - 6 > 0))
	{
		...
		{
			chat_message_img.BlitSurface(nullptr, &to_rect);
			//chat_message_img.RenderSurface(to_rect.x, to_rect.y);
			//SDL_BlitSurface(chat_message_img, nullptr, dest, &to_rect);
		}
		else
		{
...
void ZHud::RenderUnitAmountBar(SDL_Surface *dest, int off_x, int off_y)
{
	...
	/* Erst hier zuruecksetzen: ohne Grundbild waere die Marke sonst
	 * verbraucht, bevor ueberhaupt etwas gezeichnet werden konnte. */
	...
	//render text
	{
		snprintf(message, sizeof(message), "%d", unit_amount);
		...
		//if(unit_amount_text) SDL_BlitSurface(unit_amount_text, nullptr, dest, &to_rect);
		unit_amount_text.BlitSurface(nullptr, &to_rect);
...
void ZHud::RenderHealth(SDL_Surface *dest, int off_x, int off_y)
{
	...
	if(!selected_object || selected_object->IsDestroyed())
	{
		...
		health_empty_img.BlitSurface(nullptr, &to_rect);
		//health_empty_img.RenderSurface(to_rect.x, to_rect.y);
		//SDL_BlitSurface(health_empty_img, nullptr, dest, &to_rect);
	}
	else
	{
...
void ZHud::RenderTime(SDL_Surface *dest, int off_x, int off_y)
{
	...
	if(the_time > last_time || rerender_time)
	{
		...
		//render test
		//snprintf(message, sizeof(message), "%d:%02d:%02d", hours, minutes, seconds);
		//to_rect.x = off_x + 548;
		//to_rect.y = off_y;
		//surface = ZFontEngine::GetFont(BIG_WHITE_FONT).Render(message);
		//if(surface)
		//{
		//	SDL_BlitSurface(surface, nullptr, dest, &to_rect);
		//	SDL_FreeSurface(surface);
		//}

		//render hours
		if(hours <= 9)
			snprintf(message, sizeof(message), "%d", hours);
		else
		...
		surface.BlitSurface(nullptr, &to_rect);
		//surface.RenderSurface(to_rect.x, to_rect.y);
		surface.Unload();

		//render minutes
		snprintf(message, sizeof(message), "%02d", minutes);
		...
		//render seconds
		snprintf(message, sizeof(message), "%02d", seconds);
...
void ZHud::RenderBackdrop(SDL_Surface *dest, int off_x, int off_y)
{
	...
	if(rerender_backdrop)
	{
		...
		//if(render_backdrop)
		//{
		//	render_backdrop->BlitSurface(nullptr, &to_rect);
		//	//render_backdrop->RenderSurface(to_rect.x, to_rect.y);
		//	//SDL_BlitSurface(render_backdrop, nullptr, dest, &to_rect);
		//}
		//else
		//{
		//	//clear
