...
void hut_animal_graphics::LoadGraphics(int ha_num)
{
	...
	//dead images
	snprintf(filename, sizeof(filename), "assets/other/hut_animals/%s_dead_down.png", hut_animal_type_string[ha_num].c_str());
	dead_down.LoadBaseImage(filename);
	snprintf(filename, sizeof(filename), "assets/other/hut_animals/%s_dead_up.png", hut_animal_type_string[ha_num].c_str());
	...
	for(int r=0;r<MAX_ANGLE_TYPES;r++)
		...
		{
			snprintf(filename, sizeof(filename), "assets/other/hut_animals/%s_walk_r%03d_n%02d.png", hut_animal_type_string[ha_num].c_str(), ROTATION[r], i);
	...
	for(int r=0;r<MAX_ANGLE_TYPES;r+=2)
		...
		{
			snprintf(filename, sizeof(filename), "assets/other/hut_animals/%s_look_r%03d_n%02d.png", hut_animal_type_string[ha_num].c_str(), ROTATION[r], i);
...
void AHutAnimal::GotoRandomTile()
{
	...
	else
	{
		...
		{
			ZLOG("hut animal at tile %d,%d has no where to move! killing animal...\n", ctx, cty);
			killme = true;
			return;
		}

		//ZLOG("possible:%d prefered:%d\n", possible_list.size(), preferred_list.size());
