...
void ABird::Init()
{
	...
	for(i=0;i<MAX_PLANET_TYPES;i++)
		for(k=0;k<5;k++)
		{
			snprintf(filename_c, sizeof(filename_c), "assets/other/birds/bird_%s_r%03d_n%02d.png", planet_type_string[i].c_str(), ROTATION[j], k);
...
void ABird::DoRender(ZMap &the_map, SDL_Surface *dest, int shift_x, int shift_y)
{
	...
	/* NoScaler: Flughoehe flach. Der WINKEL bleibt -- bei ABird ist er die
	 * Flugrichtung (abird.cpp: dx = bird_speed*cos(PI*angle/180)), und weil
	 * dangle 0 ist, kostet er nur EIN rotozoom je Vogel. */
	the_map.RenderZSurface(render_img, x,
	                       y - ((rise - 1) * 50 * ZSDL_Surface::ArcFactor()),
	                       false, true);
...
int ABird::Process()
{
	...
	//ZLOG("%lf ... sin:%lf cos:%lf dx:%lf dy:%lf dx_dif:%lf dy_dif:%lf\n", 
	//	PI * angle / 180, 
