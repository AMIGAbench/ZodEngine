...
void BRadar::Init()
{
	...
	for(i=0;i<12;i++)
	{
		snprintf(filename_c, sizeof(filename_c), "assets/buildings/radar/box_spinner_%d.png", i);
		box_spinner[i].LoadBaseImage(filename_c);// = IMG_Load_Error(filename_c);
	}
	
	for(i=0;i<8;i++)
	{
		snprintf(filename_c, sizeof(filename_c), "assets/buildings/radar/dish_%d.png", i);
		dish[i].LoadBaseImage(filename_c);// = IMG_Load_Error(filename_c);
	}
	
	for(i=0;i<2;i++)
	{
		snprintf(filename_c, sizeof(filename_c), "assets/buildings/radar/front_light_%d.png", i);
		front_light[i].LoadBaseImage(filename_c);// = IMG_Load_Error(filename_c);
		
		snprintf(filename_c, sizeof(filename_c), "assets/buildings/radar/side_light_%d.png", i);
