...
void BRepair::Init()
{
	...
	for(i=0;i<5;i++)
	{
		snprintf(filename_c, sizeof(filename_c), "assets/buildings/repair/smoke_stack_%d.png", i);
		smoke_stack[i].LoadBaseImage(filename_c);// = IMG_Load_Error(filename_c);
	}
	
	for(i=0;i<3;i++)
	{
		snprintf(filename_c, sizeof(filename_c), "assets/buildings/repair/text_box_%d.png", i);
		text_box[i].LoadBaseImage(filename_c);// = IMG_Load_Error(filename_c);
	}
	
	for(i=0;i<2;i++)
	{
		snprintf(filename_c, sizeof(filename_c), "assets/buildings/repair/bulb_%d.png", i);
		bulb[i].LoadBaseImage(filename_c);// = IMG_Load_Error(filename_c);
		
		snprintf(filename_c, sizeof(filename_c), "assets/buildings/repair/side_light_%d.png", i);
		side_light[i].LoadBaseImage(filename_c);// = IMG_Load_Error(filename_c);
		
		snprintf(filename_c, sizeof(filename_c), "assets/buildings/repair/front_light_%d.png", i);
