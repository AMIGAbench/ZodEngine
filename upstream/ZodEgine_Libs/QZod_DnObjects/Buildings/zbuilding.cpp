...
ZBuilding::ZBuilding(ZTime *ztime_, ZSettings *zsettings_, planet_type palette_) : ZObject(ztime_, zsettings_)
{
	...
	show_time = -1;
	//show_time_img = nullptr;
}

ZBuilding::~ZBuilding()
{
	//no memory leaks -- end() statt begin(): die Schleife lief nie, jedes
	//zerstoerte Gebaeude liess seine Feuer- und Raucheffekte im Speicher liegen
...
void ZBuilding::Init()
{
	...
	{
		snprintf(filename, sizeof(filename), "assets/buildings/level_%d.bmp", i+1);
		level_img[i].LoadBaseImage(filename);// = IMG_Load_Error(filename);
	}
	
	for(i=0;i<13;i++)
	{
		snprintf(filename, sizeof(filename), "assets/buildings/exhaust_%d.png", i);
		exhaust[i].LoadBaseImage(filename);// = IMG_Load_Error(filename);
	}
	
	for(i=0;i<4;i++)
	{
		snprintf(filename, sizeof(filename), "assets/buildings/little_exhaust_%d.png", i);
...
bool ZBuilding::AddBuildingQueue(unsigned char ot, unsigned char oid, bool push_to_front)
{
	...
	//ZLOG("ZBuilding::AddBuildingQueue::adding %d %d\n", ot, oid);
...
void ZBuilding::ProcessBuildingQueueData(char *data, int size)
{
	...
	{
		ZLOG("ZBuilding::ProcessBuildingQueueData::ref_id's do not match %d vs %d\n", ref_id, data_ref_id);
		return;
	}
	...
	for(int i=0;i<queue_amount;i++)
	{
		...
		if(!AddBuildingQueue(new_unit.ot, new_unit.oid, false))
			ZLOG("ZBuilding::ProcessBuildingQueueData::could not add unit %d:%d %d\n", i, new_unit.ot, new_unit.oid);

		data += sizeof(ZBProductionUnit);
	}

	//debug
	//ZLOG("ZBuilding::ProcessBuildingQueueData::queue list now %d:", queue_list.size());
	//for(vector<ZBProductionUnit>::iterator j=queue_list.begin(); j!=queue_list.end();j++)
	//	ZLOG("%d %d, ", j->ot, j->oid);
	//ZLOG("\n");
}
...
void ZBuilding::ResetShowTime(int new_time)
{
	...
	if(new_time > -1)
	{
		...
		snprintf(message, sizeof(message), "%d:%02d", minutes, seconds);
...
void ZBuilding::DoReviveEffect()
{
	...
	//no memory leaks -- end() statt begin(): die Schleife lief nie
	for(vector<EStandard*>::iterator i=extra_effects.begin(); i!=extra_effects.end(); i++)
		delete *i;
...
void ZBuilding::ProcessBuildingsEffects(double &the_time)
{
	...
	//if(should_effects) ZLOG("should_effects:%d size:%d\n", should_effects, extra_effects.size());
