...
void BFort::Init()
{
	...
	for(i=0;i<MAX_TEAM_TYPES;i++)
		for(j=0;j<4;j++)
	{
		snprintf(filename_c, sizeof(filename_c), "assets/buildings/fort/flag_%s_n%02d.png", team_type_string[i].c_str(), j);
	...
	for(i=0;i<MAX_PLANET_TYPES;i++)
	{
		...
		//if(destroyed_overlay && base_front_destroyed[i])
		{
			...
			//SDL_BlitSurface( destroyed_overlay, nullptr, base_front_destroyed_overlay[i], nullptr);
			base_front_destroyed_overlay[i] = base_front_destroyed[i].GetBaseSurface();
			destroyed_overlay.BlitSurface(nullptr, nullptr, &base_front_destroyed_overlay[i]);
		...
		//if(destroyed_overlay && base_back_destroyed[i])
		{
			...
			//SDL_BlitSurface( destroyed_overlay, nullptr, base_back_destroyed_overlay[i], nullptr);
			base_back_destroyed_overlay[i] = base_back_destroyed[i].GetBaseSurface();
			destroyed_overlay.BlitSurface(nullptr, nullptr, &base_back_destroyed_overlay[i]);
