...
void BVehicle::Init()
{
	...
	for(i=0;i<8;i++)
	{
		snprintf(filename_c, sizeof(filename_c), "assets/buildings/vehicle/spin_%d.png", i);
		spin[i].LoadBaseImage(filename_c);// = IMG_Load_Error(filename_c);
	}
	
	for(i=0;i<4;i++)
	{
		snprintf(filename_c, sizeof(filename_c), "assets/buildings/vehicle/vent_%d.png", i);
		vent[i].LoadBaseImage(filename_c);// = IMG_Load_Error(filename_c);
	}
	
	for(i=0;i<2;i++)
	{
		snprintf(filename_c, sizeof(filename_c), "assets/buildings/vehicle/lights_%d.png", i);
		lights[i].LoadBaseImage(filename_c);// = IMG_Load_Error(filename_c);
		
		snprintf(filename_c, sizeof(filename_c), "assets/buildings/vehicle/tank_%d.png", i);
		tank[i].LoadBaseImage(filename_c);// = IMG_Load_Error(filename_c);
		
		snprintf(filename_c, sizeof(filename_c), "assets/buildings/vehicle/bulb_%d.png", i);
