...
BBridge::BBridge(ZTime *ztime_, ZSettings *zsettings_, planet_type palette_, bool is_vertical_, unsigned short extra_links_) : ZBuilding(ztime_, zsettings_, palette_)
{
	...
	//render_img = nullptr;
	//render_damaged_img = nullptr;
	//render_destroyed_img = nullptr;
	palette = palette_;
...
void BBridge::ReRenderImages()
{
	...
	{
		ZLOG("error: not able to create base bridge image for rendering\n");
		return;
	}

	if(!planet_template[palette].GetBaseSurface())
	{
		ZLOG("error: required bridge palette not loaded\n");
		return;
	}
	...
	if(is_vertical)
	{
		...
		{
			ZLOG("error: bridge dimensions incorrect\n");
			return;
		}
		...
		//copy
		//SDL_BlitSurface(render_img, nullptr, render_damaged_img, nullptr);
		//SDL_BlitSurface(render_img, nullptr, render_destroyed_img, nullptr);
		render_img.BlitSurface(nullptr, nullptr, &render_damaged_img);
		render_img.BlitSurface(nullptr, nullptr, &render_destroyed_img);
