...
void BRobot::Init()
{
	...
	for(i=0;i<MAX_PLANET_TYPES;i++)
	{
		...
		for(j=1;j<MAX_TEAM_TYPES;j++)
		{
			...
			//SDL_BlitSurface(base_color[j], nullptr, base[i][j], &dest);
			//SDL_BlitSurface(base_color_destroyed[j], nullptr, base_destroyed[i][j], &dest);
			base_color[j].BlitSurface(nullptr, &dest, &base[i][j]);
			base_color_destroyed[j].BlitSurface(nullptr, &dest, &base_destroyed[i][j]);
		}
	}
	...
	for(i=0;i<8;i++)
	{
		snprintf(filename_c, sizeof(filename_c), "assets/buildings/robot/spin_%d.png", i);
		spin[i].LoadBaseImage(filename_c);// = IMG_Load_Error(filename_c);
	}
	
	for(i=0;i<6;i++)
	{
		snprintf(filename_c, sizeof(filename_c), "assets/buildings/robot/green_box_%d.png", i);
		green_box[i].LoadBaseImage(filename_c);// = IMG_Load_Error(filename_c);
	}
	
	for(i=0;i<2;i++)
	{
		snprintf(filename_c, sizeof(filename_c), "assets/buildings/robot/light_%d.png", i);
		single_light[i].LoadBaseImage(filename_c);// = IMG_Load_Error(filename_c);
		
		snprintf(filename_c, sizeof(filename_c), "assets/buildings/robot/double_light_%d.png", i);
		double_light[i].LoadBaseImage(filename_c);// = IMG_Load_Error(filename_c);
		
		snprintf(filename_c, sizeof(filename_c), "assets/buildings/robot/robot_%d.png", i);
