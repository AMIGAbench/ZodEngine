...
ZSDL_Surface RSniper::fire[MAX_TEAM_TYPES][MAX_ANGLE_TYPES][5];

RSniper::RSniper(ZTime *ztime_, ZSettings *zsettings_) : ZRobot(ztime_, zsettings_)
{
	hover_name = "Sniper";
	object_name = "sniper";
	...
	InitTypeId(object_type, SNIPER);

	//object_id = SNIPER;
	//attack_radius = SNIPER_ATTACK_RADIUS;
	//max_health = SNIPER_MAX_HEALTH;
	//health = max_health;
...
void RSniper::Init()
{
	...
	for(i=1;i<MAX_TEAM_TYPES;i++)
		for(j=0;j<MAX_ANGLE_TYPES;j++)
			for(k=0;k<5;k++)
	{
		snprintf(filename_c, sizeof(filename_c), "assets/units/robots/grunt/fire_%s_r%03d_n%02d.png", team_type_string[i].c_str(), ROTATION[j], k);
...
void RSniper::PlaySelectedAnim(ZPortrait &portrait)
{
	if(rand()%2)
	{
		...
		portrait.StartAnim(SNIPERS_REPORTING_ANIM);
}

// SDL_Surface *RSniper::GetRender()
// {
// 	if(owner != NULL_TEAM)
// 	{
// 		switch(mode)
...
void RSniper::DoRender(ZMap &the_map, SDL_Surface *dest, int shift_x, int shift_y)
{
	int &x = loc.x;
	int &y = loc.y;
	...
	if(the_map.GetBlitInfo(x, y + submerge_amount, 16, 16 - submerge_amount, from_rect, to_rect))
	{
		...
		base_surface->BlitHitSurface(&from_rect, &to_rect, nullptr, do_hit_effect);
...
void RSniper::DoAfterEffects(ZMap &the_map, SDL_Surface *dest, int shift_x, int shift_y)
{
...
int RSniper::Process()
{
	...
	if(mode == R_ATTACKING && the_time >= next_attack_time)
	{
		...
		if(action_i >= 3)
			next_attack_time = the_time + 0.30 + ((rand() % 100) * 0.18 * 0.01);
		else
