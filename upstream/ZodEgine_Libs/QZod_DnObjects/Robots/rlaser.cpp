...
ZSDL_Surface RLaser::fire[MAX_TEAM_TYPES][MAX_ANGLE_TYPES][3];
...
void RLaser::Init()
{
	...
	for(i=1;i<MAX_TEAM_TYPES;i++)
		for(j=0;j<MAX_ANGLE_TYPES;j++)
			for(k=0;k<3;k++)
	{
		snprintf(filename_c, sizeof(filename_c), "assets/units/robots/laser/fire_%s_r%03d_n%02d.png", team_type_string[i].c_str(), ROTATION[j], k);
...
void RLaser::PlaySelectedWav()
{
	...
	//}
	//else
	//	ZMix_PlayChannel(-1, selected_robot_wav[LASER], 0);
}

void RLaser::PlaySelectedAnim(ZPortrait &portrait)
{
	if(rand()%2)
	{
		switch(rand() % 4)
		...
		portrait.StartAnim(LASERS_REPORTING_ANIM);
}

// SDL_Surface *RLaser::GetRender()
// {
// 	if(owner != NULL_TEAM)
// 	{
// 		switch(mode)
