...
void ZGFile::LoadEntryList()
{
	...
	if(!fp)
	{
		ZLOG("ZGFile::LoadEntryList: could not load file '%s'\n", ZGFILE_NAME);
		return;
	}
	...
	while(1)
	{
		...
		if(ret)
		{
			ZLOG("ZGFile::LoadEntryList: fseek error\n");
			break;
		}
	}

    ZLOG("ZGFile::LoadEntryList: entries loaded:%u\n", entry_list.size());
...
void ZGFile::AddFile(string filename, SDL_Surface *img)
{
	...
	if(!img) 
	{
		ZLOG("ZGFile::AddFile: null image for '%s'\n", filename.c_str());
		return;
	}
	...
	{
		ZLOG("ZGFile::AddFile: filename already in added '%s'\n", filename.c_str());
		return;
	}
	...
	{
		ZLOG("ZGFile::AddFile: filename too long [size:%d] for '%s'\n", filename.length(), filename.c_str());
		return;
	}
	...
	if(!fp)
	{
		ZLOG("ZGFile::AddFile: could not open file '%s'\n", ZGFILE_NAME);
		return;
	}
	...
	if(!ret)
	{
		ZLOG("ZGFile::AddFile: could not write entry\n");
	...
	if(!ret)
	{
		ZLOG("ZGFile::AddFile: could not write data\n");
		SDL_FreeSurface(write_img);
		fclose(fp);
		return;
	}
...
SDL_Surface *ZGFile::LoadFile(string filename)
{
	...
	if(e_num == -1)
	{
		ZLOG("ZGFile::LoadFile::entry not found '%s'\n", filename.c_str());
		return nullptr;
	}
	...
	if(!fp)
	{
		ZLOG("ZGFile::LoadFile: could not open file '%s'\n", ZGFILE_NAME);
		return nullptr;
	}
	...
	if(ret)
	{
		ZLOG("ZGFile::LoadFile: could not fseek to %d\n", entry_list[e_num].f_offset);
		fclose(fp);
		return nullptr;
	}
	...
	if(!read_img)
	{
		ZLOG("ZGFile::LoadFile: could not create sdl surface\n");
		fclose(fp);
		return nullptr;
	}
	...
	if(!ret)
	{
		ZLOG("ZGFile::LoadFile: could not read in sdl surface\n");
