...
void ORock::Init()
{
	...
	for(i=0;i<MAX_PLANET_TYPES;i++)
	{
		...
		{
			rock_up_left_top[i] = nullptr;
			rock_up_right_top[i] = nullptr;
			rock_down_right_top[i] = nullptr;
			rock_down_left_top[i] = nullptr;
		}
	}
}
...
void ORock::EditRockRender(ZMap &the_map, const vector<ZObject*> &object_list, ZObject *robj, bool do_add)
{
	bool **rock_list;
	int map_w, map_h;
	int x, y;
	...
	//now tell all the rocks to do their job
	{
		...
		for(vector<ZObject*>::const_iterator obj=object_list.begin(); obj!=object_list.end(); obj++)
		{
...
void ORock::SetupRockRenders(ZMap &the_map, const vector<ZObject*> &object_list)
{
	bool **rock_list;
	int map_w, map_h;
	int i, j;
