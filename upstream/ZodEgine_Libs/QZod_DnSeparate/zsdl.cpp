...
// TODO static
static bool zsdl_play_music = true;

// ** TODO nullptr shanged to nullptr  (next everywhere)
// ** TODO old style-cast shanged static_cast<>() (next everywhere)
SDL_RotoZoomSurface::SDL_RotoZoomSurface()
{
    base_surface = nullptr;
	for(int i=0;i<360;i++)
		for(int j=0;j<100;j++)
            the_surface[i][j] = nullptr;
}
...
SDL_Surface *SDL_RotoZoomSurface::GetImage(int angle, double zoom)
{
    ...
    zoom_int = static_cast<int>(std::floor(zoom * 10 + 0.5));

	if(zoom_int >= 100)
	{
		ZLOG("SDL_ZoomSurface::Requesting zoom of surface too large\n");
		zoom_int = 99;
	}
	else if(zoom_int < 0)
		zoom_int = 0;
	...
	{
		ZLOG("tried to get invalid rotation:%d\n", angle);
        return nullptr;
	}

	if(the_surface[angle][zoom_int]) return the_surface[angle][zoom_int];

    the_surface[angle][zoom_int] = rotozoomSurface(base_surface,
...
SDL_Surface *SDL_RotoZoomSurface::CreateImage(int angle, double zoom)
{
    ...
    return rotozoomSurface(base_surface, static_cast<double>(angle),
                           zoom_int / 10.0, 1);
}
...
SDL_Surface *SDL_RotateSurface::GetImage(int angle)
{
    ...
    rotated_surface[angle] = rotozoomSurface(base_surface,
                                             static_cast<double>(angle),
                                             1, 1);
...
int AngleFromLoc(float dx, float dy)
{
    ...
    if(a < 0) a += 2 * 3.14159f; // TODO + 3.14159;

	//ZLOG("a:%f\n", a);

    ret = static_cast<int>(std::floor(180.f * a / 3.14159f + 0.5f));
...
void draw_selection_box(SDL_Surface *surface, SDL_Rect dim,
                        SDL_Color color, int max_x, int max_y)
{
	const int padding = 3;
	const int the_len = 5;
    int  x_end, y_end; //x_i, y_i,
	SDL_Rect line_box;
	//int sdlmap;
    ...
    if(line_box.x + line_box.w >= max_x)
        line_box.w = static_cast<Uint16>(max_x - line_box.x);
    if(line_box.y + line_box.h >= max_y)
        line_box.h = static_cast<Uint16>(max_y - line_box.y);
...
int ZMix_PlayChannel(int ch, Mix_Chunk *wav, int repeat)
{
	...
	if(ret == -1) ZLOG("unable to play wav\n");
...
SDL_Surface *IMG_Load_Error(string filename)
{
	...
	if(!(ret = ZSDL_IMG_Load(filename.c_str()))) ZLOG("could not load:%s\n", filename.c_str());
...
Mix_Music *MUS_Load_Error(string filename)
{
	...
	if(!(ret = Mix_LoadMUS(filename.c_str()))) ZLOG("could not load:%s\n", filename.c_str());
...
Mix_Chunk *MIX_Load_Error(string filename)
{
	...
	if(!(ret = Mix_LoadWAV(filename.c_str()))) ZLOG("could not load:%s\n", filename.c_str());
...
SDL_Surface *CopyImage(SDL_Surface *original)
{
    ...
    SDL_BlitSurface(original, nullptr, copy, nullptr);
...
SDL_Surface *CopyImageShifted(SDL_Surface *original, int x, int y)
{
    ...
    SDL_BlitSurface(original, nullptr, copy, &to_rect);
