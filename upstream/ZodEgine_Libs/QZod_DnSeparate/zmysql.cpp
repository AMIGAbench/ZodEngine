...
ZMysql::ZMysql()
{
    ...
    mysql_conn = nullptr; // TODO nullptr;
#endif
}
...
bool ZMysql::LoadDetails(string &error_msg, string filename)
{
    ...
    std::cerr<<"\n--<error>\t"<< error_msg<<"\t"<<filename<<std::endl;
	return false;
#else
	error_msg = "mysql not supported with this executable, please recompile with mysql support.";
	return false;
...
{
    error_msg.clear();     // TODO use for return
#ifndef DISABLE_MYSQL

	//disconnect if already existing
    if(mysql_conn){
        Disconnect(error_msg);
        if( !error_msg.empty() ){
	...
	//init
    mysql_conn = mysql_init(nullptr); // TODO nullptr);

    // ** TODO change logical for error_msg
	//it create?
	if(!mysql_conn)
	{
	...
	//connect
    else if( !mysql_real_connect(mysql_conn, host_name.c_str(),
                          login_name.c_str(), login_password.c_str(),
                          database_name.c_str(),
                          0, nullptr, CLIENT_FOUND_ROWS))// TODO nullptr
	{
...
#else
    error_msg = "mysql not supported with this executable, "
            "please recompile with mysql support.";
#endif
...
bool ZMysql::CreateUserTable(string &error_msg)
{
    ...
    tabID int NOT nullptr AUTO_INCREMENT,\
    PRIMARY KEY(tabID),\
	...
	if(!mysql_conn && !Connect(error_msg)) 
        error_msg= "(mysql_conn not connect) "+ error_msg;
	//send command
    else if( mysql_query(mysql_conn, sql_cmd.c_str()))
...
bool ZMysql::AddUser(string &error_msg, ZMysqlUser &nu, bool &user_added)
{
	...
	*/
	...
	snprintf(sql_cmd_c, sizeof(sql_cmd_c),"INSERT INTO %s \
	...
	{
		//successful
		ZLOG("successful mysql user add:'%s'\n", nu.username.c_str());
		user_added = true;
		return true;
	}
	else
...
bool ZMysql::CheckUserExistance(string &error_msg, ZMysqlUser &user, bool &user_exists)
{
	...
	//format
	snprintf(sql_cmd_c, sizeof(sql_cmd_c), "SELECT * FROM %s WHERE (username = '%s' OR loginname = '%s')",
		users_table.c_str(),
		user.username.c_str(),
    ...
    // return true;    TODO! never executable
#else
	error_msg = "mysql not supported with this executable, please recompile with mysql support.";
	return false;
#endif
...
bool ZMysql::LoginUser(string &error_msg, string loginname, string password, ZMysqlUser &user, bool &user_found)
{
	...
	//format
	snprintf(sql_cmd_c, sizeof(sql_cmd_c), "SELECT * FROM %s WHERE (loginname = '%s' AND password = '%s')",
		users_table.c_str(),
		loginname.c_str(),
		password.c_str());
...
bool ZMysql::UpdateUserVariable(string &error_msg, int tabID, string varname, string value)
{
	...
	//format
	snprintf(sql_cmd_c, sizeof(sql_cmd_c), "UPDATE %s SET %s='%s' WHERE tabID='%d'",
		users_table.c_str(),
		varname.c_str(),
		value.c_str(),
		tabID);
...
bool ZMysql::IncreaseUserVariable(string &error_msg, int tabID, string varname, int amount)
{
	...
	//format
	snprintf(sql_cmd_c, sizeof(sql_cmd_c), "UPDATE %s SET %s=%s+%d WHERE tabID='%d'",
		users_table.c_str(),
		varname.c_str(),
		varname.c_str(),
		amount,
...
bool ZMysql::IPInUserTable(string &error_msg, string ip, int &users_found)
{
	...
	//format
	snprintf(sql_cmd_c, sizeof(sql_cmd_c), "SELECT * FROM %s WHERE (creation_ip = '%s' OR last_ip = '%s')",
		users_table.c_str(),
		ip.c_str(),
		ip.c_str());
...
bool ZMysql::InsertOnlineHistoryEntry(string &error_msg, int time_stamp, int player_count, int tray_player_count)
{
	...
	//insert
	{
		snprintf(sql_cmd_c, sizeof(sql_cmd_c),"INSERT INTO %s ( time_stamp, player_count, tray_player_count ) VALUES ('%d', '%d', '%d')",
	...
	//reduce size
	{
		...
		//format
		snprintf(sql_cmd_c, sizeof(sql_cmd_c), "SELECT * FROM %s ORDER BY tabID ASC", online_history_table.c_str());
		...
		if(!mysql_query(mysql_conn, sql_cmd_c))
		{
			...
			for(int i=0; i < row_count - max_entries; i++)
			{
				...
				snprintf(sql_cmd_c, sizeof(sql_cmd_c),"DELETE FROM %s WHERE tabID='%s'",
					online_history_table.c_str(),
					row[0]);
...
bool ZMysql::GetAffiliateId(string &error_msg, string ip, int &aff_id, bool &ip_found)
{
	...
	//format
	snprintf(sql_cmd_c, sizeof(sql_cmd_c), "SELECT * FROM %s WHERE ip = '%s'",
