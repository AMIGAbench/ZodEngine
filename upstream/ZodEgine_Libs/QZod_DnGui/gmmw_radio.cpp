...
void GMMWRadio::CheckSI()
{
	...
	{
		ZLOG("GMMWRadio::CheckSI:bad si:%d max_selections:%d\n", si, selections);
		si = 0;
	}
}
...
void GMMWRadio::DoRender(ZMap &the_map, SDL_Surface *dest, int tx, int ty)
{
	...
	{
		//left
		radio_left_img.BlitSurface(nullptr, tx, ty);

		//right
		radio_right_img.BlitSurface(nullptr, tx + (w - MMRADIO_RIGHT_WIDTH), ty);

		//center
		for(int i=0;i<selections-2;i++)
			radio_center_img.BlitSurface(nullptr, tx + MMRADIO_LEFT_WIDTH + (i * MMRADIO_CENTER_WIDTH), ty);
	}
	...
	//render selector
	{
		...
		radio_selector_img.BlitSurface(nullptr, tx + ix, ty + 1);
	}
}
