...
protected:
	...
	//Setzt den Fensterrahmen einmal in base_img zusammen (siehe dort).
	void BuildBaseImage();
	...
	/* Fertig zusammengesetzter Fensterrahmen.
	 *
	 * RenderBase setzte den Rahmen bisher in JEDEM Bild aus Einzelkacheln neu
	 * zusammen -- die Innenflaeche ueber eine verschachtelte Schleife, also
	 * hunderte kleiner Blits je Bild fuer ein Bild, das sich nie aendert.
	 * Genau das hat der Nutzer als starkes Ruckeln bei offenem Menue gemeldet.
	 *
	 * Der Rahmen wird jetzt einmal hier hinein gezeichnet und je Bild mit
	 * einem einzigen Blit ausgegeben. Alle acht Rahmenbilder sind nachweislich
	 * vollstaendig deckend (Alpha durchgehend 255), die Flaeche wird deshalb
	 * deckend angelegt -- damit ist es ein reiner Kopierblit statt einer
	 * Alphamischung.
	 *
	 * Verworfen wird der Zwischenspeicher anhand seiner EIGENEN Groesse gegen
	 * w/h. Das ist Absicht: Mehrere Unterklassen setzen h nachtraeglich neu
	 * (gmm_change_teams gleich an drei Stellen), und da sie direkt auf die
	 * geschuetzten Felder schreiben, laesst sich in keine Zuweisung einhaengen.
	 * So kann der Zwischenspeicher nicht unbemerkt veralten. */
	ZSDL_Surface base_img;
	gmm_flag gmm_flags;
