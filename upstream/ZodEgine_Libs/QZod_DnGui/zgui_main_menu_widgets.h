...
public:
	...
	/* Hier standen Pruefungen auf negative Koordinaten, die bei jedem Treffer
	 * eine Zeile ueber std::cerr schrieben. Sie sind aus zwei Gruenden raus:
	 *
	 * 1. Sie sind sachlich falsch. ZGuiMainMenuBase::Motion uebergibt
	 *    tx = x_ - x, also den Zeiger RELATIV zur Fensterecke. Liegt der
	 *    Zeiger links oder oberhalb des Menues -- bei einem Menue in der
	 *    Bildmitte der Normalfall --, ist der Wert negativ. Gemeldet wurde
	 *    also gewoehnlicher Betrieb, kein Fehler.
	 * 2. Sie waren teuer. std::cerr laeuft am Logmodul vorbei und landet
	 *    unumgeleitet auf der Konsole; auf AmigaOS ist jede Zeile ein
	 *    synchroner Schreibvorgang. Bei 15 Widgets (gmm_change_teams) sind
	 *    das 15 formatierte iostream-Ausgaben je Mausbewegung, jede mit
	 *    __FILE__ als vollem Pfad.
	 *
	 * Genau das hat der Nutzer gemeldet: starkes Ruckeln, aber nur bei
	 * offenem Menue und nur, solange der Zeiger NICHT ueber dem Menue liegt. */
	virtual void DoRender(ZMap &the_map, SDL_Surface *dest, int tx, int ty)
	{
		(void)the_map; (void)dest; (void)tx; (void)ty;
	}
	...
	/* Auch hier die cerr-Pruefungen entfernt, siehe DoRender oben. Motion ist
	 * die teuerste davon: ZGuiMainMenuBase::Motion ruft sie bei JEDER
	 * Mausbewegung fuer JEDES Widget jedes offenen Menues. */
	virtual bool Click(int x_, int y_)   { (void)x_; (void)y_; return false; }
	virtual bool UnClick(int x_, int y_) { (void)x_; (void)y_; return false; }
	virtual bool Motion(int x_, int y_)  { (void)x_; (void)y_; return false; }
	virtual bool KeyPress(int c)         { (void)c; return false; }
