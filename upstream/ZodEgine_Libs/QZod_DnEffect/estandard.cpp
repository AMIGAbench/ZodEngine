...
void EStandard::Init()
{
	...
	for(i=0;i<4;i++)
		{
			snprintf(filename_c, sizeof(filename_c), "assets/units/vehicles/death_effects/big_smoke_n%02d.png", i);
			edeath_big_smoke[i].LoadBaseImage(filename_c);// = IMG_Load_Error(filename_c);

			snprintf(filename_c, sizeof(filename_c), "assets/units/vehicles/death_effects/little_fire_n%02d.png", i);
			edeath_little_fire[i].LoadBaseImage(filename_c);// = IMG_Load_Error(filename_c);

			snprintf(filename_c, sizeof(filename_c), "assets/units/vehicles/death_effects/small_fire_smoke_n%02d.png", i);
			edeath_small_fire_smoke[i].LoadBaseImage(filename_c);// = IMG_Load_Error(filename_c);

			snprintf(filename_c, sizeof(filename_c), "assets/units/vehicles/death_effects/fire_n%02d.png", i);
