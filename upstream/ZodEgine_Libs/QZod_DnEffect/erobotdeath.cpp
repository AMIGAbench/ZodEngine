...
void ERobotDeath::Init()
{
	...
	for(i=1;i<MAX_TEAM_TYPES;i++)
	{
		for(j=0;j<4;j++)
			...
			{
				snprintf(filename_c, sizeof(filename_c), "assets/units/robots/die%d_%s_n%02d.png", j+1, team_type_string[i].c_str(), k);
		...
		for(k=0;k<17;k++)
		{
			snprintf(filename_c, sizeof(filename_c), "assets/units/robots/melt_%s_n%02d.png", team_type_string[i].c_str(), k);
