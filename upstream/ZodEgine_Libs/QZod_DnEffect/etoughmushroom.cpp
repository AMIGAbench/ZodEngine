...
EToughMushroom::EToughMushroom(ZTime *ztime_, int x_, int y_, double size) : ZEffect(ztime_)
{
	...
	/* NoScaler: auch die Zentrierung und der Zeilenversatz haengen an
	 * zoom_size (siehe unten x/y und DoRender). Bliebe der Wert bei 1,3,
	 * waehrend das Bild unskaliert kommt, sitzt die Wolke daneben. */
	zoom_size = ZSDL_Surface::NoScaler() ? 1.0 : size;
	//size *= 10;
	//zoom_size = (int)size;
	//zoom_size /= 10;
...
void EToughMushroom::Init()
{
	...
	for(i=0;i<12;i++)
		{
			snprintf(filename_c, sizeof(filename_c), "assets/units/robots/tough/mushroom_n%02d.png", i);
