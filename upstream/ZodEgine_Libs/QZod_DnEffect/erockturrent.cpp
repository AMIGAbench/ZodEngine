...
void ERockTurrent::Init()
{
	...
	for(i=0;i<MAX_PLANET_TYPES;i++)
	{
		...
		for(j=0;j<12;j++)
		{
			snprintf(filename_c, sizeof(filename_c), "assets/planets/rock_effects/debri_large%d_%s_n%02d.png", k, planet_type_string[i].c_str(), j);
...
void ERockTurrent::Process()
{
...

	{
		...
		size += 1;
		y -= (size-1) * 30 * ZSDL_Surface::ArcFactor();
...
void ERockTurrent::DoRender(ZMap &zmap, SDL_Surface *dest)
{
	...
	/* NoScaler: Truemmer taumeln ueber ihre Bildfolge (render_i), genau
	 * wie im Original. Der je Bild NEUE Winkel ist Nighsofts Zutat und
	 * kostet in jedem Bild eine neu gerechnete Drehflaeche. Das einmalige
	 * Drehen der Geschosse (Flugrichtung) bleibt dagegen erhalten. */
