...
void ERockParticle::Init()
{
	...
	for(i=0;i<MAX_PLANET_TYPES;i++)
	{
		for(j=0;j<8;j++)
		{
			snprintf(filename_c, sizeof(filename_c), "assets/planets/rock_effects/debri_mid0_%s_n%02d.png", planet_type_string[i].c_str(), j);
			debri_mid0_img[i][j].LoadBaseImage(filename_c);

			snprintf(filename_c, sizeof(filename_c), "assets/planets/rock_effects/debri_mid1_%s_n%02d.png", planet_type_string[i].c_str(), j);
			debri_mid1_img[i][j].LoadBaseImage(filename_c);
		}

		for(j=0;j<16;j++)
		{
			snprintf(filename_c, sizeof(filename_c), "assets/planets/rock_effects/debri_small_%s_n%02d.png", planet_type_string[i].c_str(), j);
...
void ERockParticle::Process()
{
...

	{
		...
		size += 1;
		y -= (size-1) * 150 * ZSDL_Surface::ArcFactor();
	}
}
