...
void ETankSmoke::Init()
{
	...
	for(int r=0;r<MAX_ANGLE_TYPES;r++)
		for(int i=0;i<7;i++)
		{
			snprintf(filename, sizeof(filename), "assets/units/vehicles/track_dust_r%03d_n%02d.png", ROTATION[r], i);
			tank_smoke[r][i].LoadBaseImage(filename);

			if(i<4)
			{
				snprintf(filename, sizeof(filename), "assets/units/vehicles/track_spark_r%03d_n%02d.png", ROTATION[r], i);
