...
void ETurrentMissile::Init()
{
	...
	for(i=0;i<4;i++)
	{
		snprintf(filename_c, sizeof(filename_c), "assets/other/grenades/grenade_n%02d.png", i);
		grenade_img[i].LoadBaseImage(filename_c);
	}

	for(i=0;i<8;i++)
	{
		snprintf(filename_c, sizeof(filename_c), "assets/units/vehicles/light/top_pop_n%02d.png", i);
		light_turrent[i].LoadBaseImage(filename_c);

		snprintf(filename_c, sizeof(filename_c), "assets/units/vehicles/medium/top_pop_n%02d.png", i);
		medium_turrent[i].LoadBaseImage(filename_c);

		for(j=1;j<MAX_TEAM_TYPES;j++)
		{
			snprintf(filename_c, sizeof(filename_c), "assets/units/vehicles/heavy/top_pop_%s_n%02d.png", team_type_string[j].c_str(), i);
	...
	for(i=0;i<2;i++)
		for(j=0;j<12;j++)
		{
			snprintf(filename_c, sizeof(filename_c), "assets/buildings/death_effects/piece%d_n%02d.png", i, j);
	...
	for(i=0;i<5;i++)
		for(j=0;j<12;j++)
		{
			snprintf(filename_c, sizeof(filename_c), "assets/buildings/death_effects/fort_piece%d_n%02d.png", i, j);
...
void ETurrentMissile::Process()
{
...

	{
		...
		size += 1.0;
		/* Urfassung: "y -= size*30; y += 30;" -- rechnerisch dasselbe
		 * wie "y -= (size-1)*30", weil sich das +1 und die +30 aufheben.
		 * Zusammengefasst, damit der Faktor genau den Bogenanteil trifft. */
		y -= (size - 1.0) * 30 * ZSDL_Surface::ArcFactor();
...
void ETurrentMissile::DoRender(ZMap &zmap, SDL_Surface *dest)
{
	...
	/* NoScaler: siehe erockturrent.cpp -- je Bild neu gedreht. */
	/* NoSpin: nur das je Bild neue Drehen entfaellt -- Groesse und
	 * Flugbogen bleiben. Siehe zsdl_opengl.h. */
	if(!ZSDL_Surface::NoScaler() && !ZSDL_Surface::NoSpin())
